...
class CustomercreateForm(forms.ModelForm):
    def __init__(self,branch, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['sales_staff'].queryset = CustomUser.objects.filter(is_active=True, branch_id=branch, designation_id__designation_name__in=["Sales Executive", "Marketing Executive", "Salesman"])
...
class CustomerEditForm(forms.ModelForm):
    def __init__(self,branch, *args, **kwargs):
        super().__init__(*args, **kwargs)
        current_staff_id = self.instance.sales_staff_id if self.instance and hasattr(self.instance, 'sales_staff_id') else None
        
        queryset = CustomUser.objects.filter(
            is_active=True,
            branch_id=branch,
            designation_id__designation_name__in=["Sales Executive", "Marketing Executive", "Salesman"]
        )
        if current_staff_id:
            queryset = queryset | CustomUser.objects.filter(id=current_staff_id)
