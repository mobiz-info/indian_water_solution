...
from accounts.templatetags.accounts_templatetags import get_missed_customer_count
...
class Inactive_Customer_List(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if route_name:
            ...
            route_customers = Customers.objects.filter(is_guest=False, is_deleted=False, is_active=True)
            
            all_todays_customer_ids = set()
            for route_obj in RouteMaster.objects.all():
                todays_customers = find_customers(request, str(today), route_obj.pk) or []
                for customer in todays_customers:
                    all_todays_customer_ids.add(customer['customer_id'])

            inactive_customers = route_customers.exclude(pk__in=visited_customers).exclude(pk__in=all_todays_customer_ids)
...
def load_route_salesman(request):
    route_id = request.GET.get('route_id')
    salesman_id = None
    if route_id:
        van_route = Van_Routes.objects.filter(routes__route_id=route_id).first()
        if van_route and van_route.van and van_route.van.salesman:
            salesman_id = van_route.van.salesman.id
    return JsonResponse({'salesman_id': salesman_id})
...
def edit_customer(request,pk):
    ...
    form = CustomerEditForm(cust_Data.branch_id,instance = cust_Data)
    template_name = 'accounts/edit_customer.html'
    context = {"form":form}
    try:
        if request.method == 'POST':
            form = CustomerEditForm(cust_Data.branch_id,instance = cust_Data,data = request.POST)
            context = {"form":form}
...
def missing_customers_excel(request):
    request_date = request.GET.get('date')
    if request_date:
        request_date = datetime.strptime(request_date, '%Y-%m-%d').date()
    else:
    ...
    for route in routes:
        missed_data = get_missed_customer_count(request, route.pk, request_date)

        ws.append([
            row_no,
            route.route_name,
            missed_data['actual_customers'],
            missed_data['planned_visitors'],
            missed_data['missed_customers'],
            missed_data['supplied_customers_from_schedule'],
            missed_data['supplied_customers_from_clients'],
        ])
        row_no += 1
    ...
    response['Content-Disposition'] = (
        f'attachment; filename=missed_customers_{request_date}.xlsx'
    )
