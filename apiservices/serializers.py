...
class CustomerOutstandingSerializer(serializers.ModelSerializer):
    ...
    def get_amount(self, obj):
        ...
        return get_customer_outstanding_amount(obj, salesman=salesman)
    # def get_amount(self,obj):
...
class CollectionCustomerSerializer(serializers.ModelSerializer):
    ...
    def get_invoices(self, obj):
        ...
        try:
            ...
            invoices = Invoice.objects.filter(**invoice_filters).order_by('-created_date')
            for invoice in invoices:
                invoice_data = {
                    'invoice_id': str(invoice.id),
                    'created_date': serializers.DateTimeField().to_representation(invoice.created_date),
        ...
        try:
            ...
            with transaction.atomic():
                ...
                supply_exists = CustomerSupply.objects.filter(
                    ...
                    created_date__gte=django_timezone.now() - timedelta(minutes=1)
                ).exists()
                ...
                if total_fivegallon_qty > 0:
                    ...
                    if (existing_custody + existing_outstanding) == 0:
                        customer_supply.allocate_bottle_to_custody = int(total_fivegallon_qty)
