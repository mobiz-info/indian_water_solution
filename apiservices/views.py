            ...
            if username and password:
                ...
                if user is not None:
                    if user.is_active:
                        ...
                        try:
                            from master.models import AppPermission
                            rate_edit_perm = AppPermission.objects.filter(permission_name="Customer Rate Edit", is_active=True).first()
                            rate_edit_routes = [str(rid) for rid in rate_edit_perm.routes.values_list('route_id', flat=True)] if rate_edit_perm else []
                        except Exception:
                            rate_edit_routes = []
                    ...
                    return Response({
                        ...
                        'data': data, 
                        'app_permissions': {
                            'customer_rate_edit_routes': rate_edit_routes
                        },
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            with transaction.atomic():
                ...
                # --- Zero Bottle Status Auto-Custody Fix ---
                # If this customer currently has ZERO bottles (no custody + no outstanding empty cans),
                # the entire 5-gallon sale quantity automatically becomes their bottle status (custody).
                if total_fivegallon_qty > 0:
                    existing_custody = CustodyCustomItems.objects.filter(
                        custody_custom__customer=customer_supply.customer,
                        product__product_name__in=["5 Gallon", "5 Gallon Empty Bottle"],
                    ).aggregate(total=Sum('quantity'))['total'] or 0

                    existing_outstanding = OutstandingProduct.objects.filter(
                        customer_outstanding__customer=customer_supply.customer,
                        customer_outstanding__product_type='emptycan',
                    ).aggregate(total=Sum('empty_bottle'))['total'] or 0

                    if (existing_custody + existing_outstanding) == 0:
                        allocate_bottle_to_custody = int(total_fivegallon_qty)
                        allocate_bottle_to_pending = 0
                        allocate_bottle_to_paid = 0
                        customer_supply.allocate_bottle_to_custody = allocate_bottle_to_custody
                        customer_supply.allocate_bottle_to_pending = allocate_bottle_to_pending
                        customer_supply.allocate_bottle_to_paid = allocate_bottle_to_paid
                        customer_supply.save()
                # -------------------------------------------
    ...
    def post(self, request,pk, *args, **kwargs):
        try:
            with transaction.atomic():
                ...
                # --- Zero Bottle Status Auto-Custody Fix ---
                # If this customer currently has ZERO bottles, the entire 5-gallon qty becomes their bottle status.
                _zero_bottle_fix_applied = False
                if total_fivegallon_qty > 0:
                    existing_custody = CustodyCustomItems.objects.filter(
                        custody_custom__customer=customer_supply.customer,
                        product__product_name__in=["5 Gallon", "5 Gallon Empty Bottle"],
                    ...
                    if (existing_custody + existing_outstanding) == 0:
                        _zero_bottle_fix_applied = True
                        _auto_custody_qty = int(total_fivegallon_qty)
                        _custody_instance = CustodyCustom.objects.create(
                            customer=customer_supply.customer,
                            created_by=request.user.id,
                            created_date=datetime.today(),
                            deposit_type="non_deposit",
                        ...
                        CustodyCustomItems.objects.create(
                            product=ProdutItemMaster.objects.get(product_name="5 Gallon"),
                            quantity=_auto_custody_qty,
                            custody_custom=_custody_instance
                        )
                        _custody_stock, _ = CustomerCustodyStock.objects.get_or_create(
                            customer=customer_supply.customer,
                            product=ProdutItemMaster.objects.get(product_name="5 Gallon"),
                        )
                        _custody_stock.quantity += _auto_custody_qty
                        _custody_stock.save()
                        customer_supply.allocate_bottle_to_custody = _auto_custody_qty
                        customer_supply.allocate_bottle_to_pending = 0
                        customer_supply.allocate_bottle_to_paid = 0
                        customer_supply.custody_added = True
                        customer_supply.save()
                # -------------------------------------------

                # empty bottle calculate
                if _zero_bottle_fix_applied:
                    pass  # Bottles went to custody — no outstanding empty cans to create
...
class customer_outstanding(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Calculate totals using single aggregated queries (avoids N+1 query problem)
        customer_pks = list(customers.values_list('pk', flat=True))

        # Bulk invoice outstanding per customer
        from invoice_management.models import Invoice
        from client_management.models import CustomerCredit
        from django.db.models import ExpressionWrapper
        invoice_totals = Invoice.objects.filter(
            customer__pk__in=customer_pks,
            salesman=request.user,
            is_deleted=False
        ...
        invoice_map = {item['customer__pk']: item['outstanding'] or 0 for item in invoice_totals}

        credit_totals = CustomerCredit.objects.filter(
            customer__pk__in=customer_pks,
            salesman=request.user
        ).values('customer__pk').annotate(total=Sum('amount'))
        credit_map = {item['customer__pk']: item['total'] or 0 for item in credit_totals}

        total_outstanding_amount = sum(
            (invoice_map.get(pk, 0) - credit_map.get(pk, 0)) for pk in customer_pks
        )

        total_outstanding_bottles = OutstandingProduct.objects.filter(
            customer_outstanding__customer__pk__in=customer_pks,
...
class CollectionAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if customers.exists():
            ...
            collection_serializer = CollectionCustomerSerializer(customers, many=True, context={'request': request})
            return Response({
                'status': True,
        ...
        try:
            with transaction.atomic():
                ...
                invoices = (
                    Invoice.objects
                    .select_for_update()
                    .filter(pk__in=invoice_ids, customer=customer, is_deleted=False,salesman=request.user)
                ...
                customer_credit = (
                    ...
                    .filter(customer=customer, salesman=request.user)
                ...
                # ---------------- CHEQUE: save as pending, NO CollectionPayment yet ----------------
                if payment_method.lower() == "cheque":
                    from sales_management.models import PendingChequeCollection
                    pending = PendingChequeCollection.objects.create(
                        customer=customer,
                        salesman=request.user,
                    ...
                    pending.invoices.set(invoices)
                    return Response({"message": "Cheque payment saved. Pending clearance."}, status=201)

                # ---------------- CREATE COLLECTION (non-cheque only) ----------------
                collection_payment = CollectionPayment.objects.create(
                    payment_method=payment_method.upper(),
                    customer=customer,
                    salesman=request.user,
                ...
                return Response({
                    ...
                    "credit_balance": CustomerCredit.objects.filter(customer=customer, salesman=request.user)
...
class DashboardAPI(APIView):
    ...
    def get(self, request, route_id, trip):
        ...
        supplied_customers_count = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date).count()
        ...
        # sales records start
        cash_sale_total_amount = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date,amount_recieved__gt=0).aggregate(total_amount=Sum('subtotal'))['total_amount'] or 0
        ...
        cash_sale_amount_recieved = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date,amount_recieved__gt=0).aggregate(total_amount=Sum('amount_recieved'))['total_amount'] or 0
        ...
        credit_sale_total_amount = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date,amount_recieved__lte=0).aggregate(total_amount=Sum('subtotal'))['total_amount'] or 0
        ...
        credit_sale_amount_recieved = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date,amount_recieved__lte=0).aggregate(total_amount=Sum('amount_recieved'))['total_amount'] or 0
        ...
        # sales records end
        # cash in hand start
        cash_sale_amount = CustomerSupply.objects.filter(customer__routes__pk=route_id,supply_date__date=date,amount_recieved__gt=0).aggregate(total_amount=Sum('subtotal'))['total_amount'] or 0
        ...
        total_fivegallon_supplied = CustomerSupplyItems.objects.filter(customer_supply__customer__routes__pk=route_id,customer_supply__supply_date__date=date).aggregate(total_qty=Sum('quantity'))['total_qty'] or 0
        data = {
            'date': date_str,
            'today_schedule': {
...
class SalesInvoicesAPIView(APIView):
    ...
    def get(self, request):
        data = request.data
        # Accept from query params first (reliable for GET), fallback to body
        start_date = request.GET.get('start_date') or data.get('start_date')
        end_date = request.GET.get('end_date') or data.get('end_date')
        sales_type = request.GET.get('invoice_types') or data.get('invoice_types')
        route_name = request.GET.get('route_id') or data.get('route_id')
        ...
        if route_name:
            # Specific route requested (from settings selection)
            sales_filter["customer__routes__route_id"] = route_name
        elif Van_Routes.objects.filter(van__salesman=request.user).exists():
            # Salesman: filter by their assigned salesman field
            sales_filter["salesman"] = request.user
        else:
            # Admin/non-salesman: show all routes
        ...
        response_data = {
            ...
            "data": {
                ...
                "total_amount_collected": total_amount_collected,
                "filters": {
                    "route_name": route_name if route_name else "All Routes",
            ...
            with transaction.atomic():
                ...
                if audit_details_instances.exists():
                    for audit_detail in audit_details_instances:
                        ...
                        outstanding_amount = Decimal(outstanding_amount) - Decimal(collection_amount)
                        
                        ad_outstanding_amount = audit_detail.outstanding_amount or Decimal('0.00')
                        audit_variation_amount = Decimal(ad_outstanding_amount) - outstanding_amount
                        
                        if (audit_variation_amount != 0) and (outstanding_amount != audit_detail.outstanding_amount):
                            print("working audit")
                            print("ad_outstanding_amount")
                            print(ad_outstanding_amount)
                            print("==================================")
                            print("audit variation amount")
                            print(audit_variation_amount)
                            print("==================================")

                            if audit_variation_amount > 0:
                                print("variation amount greater")
                                # ── CASE 1: Customer owes MORE than system recorded ──
                                # Resolve the salesman from the customer's route:
                                # Specifically pick the van on this route whose salesman has user_type='Salesman'
                                _route = audit_detail.customer.routes
                                _van_route = Van_Routes.objects.filter(
                                    routes=_route,
                                    van__salesman__user_type='Salesman'
                                ).select_related("van__salesman").first()
                                _invoice_salesman = _van_route.van.salesman if _van_route else user

                                print("invoice salesman")
                                print(_invoice_salesman)

                                # Create a new outstanding record and a new credit invoice.
                                customer_outstanding_amount = CustomerOutstanding.objects.create(
                                    product_type="amount",
                                    created_by=_invoice_salesman.id if _invoice_salesman else request.user.id,
                                    customer=audit_detail.customer,
                                    created_date=datetime.today(),
                                    outstanding_date=datetime.today(),
                                )
                                ...
                                # Create the invoice — always credit_invoice + non_paid (matches create_customer_outstanding)
                                invoice = Invoice.objects.create(
                                    created_date=datetime.today(),
                                    ...
                                    amout_recieved=0,
                                    customer=audit_detail.customer,
                                    reference_no="Outstanding added from audit",
                                    salesman=_invoice_salesman,
                                    invoice_type="credit_invoice",
                                    invoice_status="non_paid",
                                )
                                customer_outstanding_amount.invoice_no = invoice.invoice_no
                                customer_outstanding_amount.save(update_fields=["invoice_no", "outstanding_date"])
                                ...
                                InvoiceItems.objects.create(
                                    ...
                                    invoice=invoice,
                                    remarks="Outstanding added from audit"
                                )

                                # Rebuild CustomerOutstandingReport from invoices (matches create_customer_outstanding)
                                ...
                                # ── CASE 2: Customer owes LESS than system recorded ──
                                # Apply the credit as a payment against existing non-paid invoices
                                credit_to_apply = Decimal(str(abs(audit_variation_amount)))
                                ...
                                for inv in unpaid_invoices:
                                    ...
                                    apply_amount = min(credit_to_apply, balance)

                                    inv.amout_recieved += apply_amount
                                    if inv.amout_recieved >= inv.amout_total:
                                        inv.amout_recieved = inv.amout_total
                                        inv.invoice_status = "paid"
                                    inv.save(update_fields=["amout_recieved", "invoice_status"])

                                    # Record the adjustment in outstanding (negative amount = credit)
                                    customer_outstanding_credit = CustomerOutstanding.objects.create(
                                        product_type="amount",
                                        created_by=request.user.id,
                                        customer=audit_detail.customer,
                                    ...
                                    OutstandingAmount.objects.create(
                                        customer_outstanding=customer_outstanding_credit,
                                        amount=-apply_amount,
                                    )

                                    credit_to_apply -= apply_amount

                                # Update the CustomerOutstandingReport summary
                                total_credit = Decimal(str(abs(audit_variation_amount))) - credit_to_apply
                                if total_credit > 0:
                                    try:
                                        outstanding_instance = CustomerOutstandingReport.objects.get(customer=audit_detail.customer, product_type="amount")
                                        outstanding_instance.value -= total_credit
                                        outstanding_instance.save()
                                    except CustomerOutstandingReport.DoesNotExist:
                                        pass
                                
                        ad_bottle_outstanding = audit_detail.bottle_outstanding or 0
                        if ad_bottle_outstanding != 0 :
                            ...
                            audit_variation_bottle_count = ad_bottle_outstanding - total_bottles
                            
                            if total_bottles != ad_bottle_outstanding:
                                customer_outstanding_empty_can = CustomerOutstanding.objects.create(
                                    customer=audit_detail.customer,
                                    product_type="emptycan",
                                    created_by=request.user.id,
                        ...
                        ad_outstanding_coupon = audit_detail.outstanding_coupon or 0
                        if ad_outstanding_coupon != 0:
                            ...
                            if total_coupons != ad_outstanding_coupon:
                            
                                audit_variation_coupon_count = ad_outstanding_coupon - total_coupons
...
class UpdateCustomerRateAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, customer_id):
        try:
            ...
            if new_rate is not None:
                old_rate = customer.rate
                customer.rate = new_rate
                customer.save()
                ...
                try:
                    ...
                    CustomerPriceChange.objects.create(
                        customer=customer,
                        created_by=request.user.username if hasattr(request, 'user') and request.user else "App User",
                        old_price=old_rate if old_rate else 0.0,
                        new_price=new_rate
                    )
                except Exception as e:
                    print(f"Error creating price history: {e}")

                return Response({"message": "Rate updated successfully", "rate": customer.rate}, status=status.HTTP_200_OK)
...
class OverviewAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        todays_supply_instances = CustomerSupply.objects.filter(supply_date__date=date)
...
class GetAppPermissionsAPI(APIView):
    permission_classes = [IsAuthenticated]
