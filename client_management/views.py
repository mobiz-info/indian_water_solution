...
def create_customer_supply(request,pk):
    ...
    SupplyItemsFormset = formset_factory(CustomerSupplyItemsForm, extra=1)
    message = ''
    ...
    if request.method == 'POST':
        ...
        if customer_supply_form.is_valid() and customer_supply_items_formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    supply_instance.salesman = salesman
                    supply_instance.created_by = salesman.pk
        ...
        # Default the first item to "5 Gallon" product
        try:
            from product.models import ProdutItemMaster
            five_gallon_product = ProdutItemMaster.objects.filter(product_name__icontains="5 Gallon").first()
            initial_data = [{'product': five_gallon_product.pk}] if five_gallon_product else []
        except Exception as e:
            print("Error finding 5 Gallon:", e)
            initial_data = []
        extra_forms = 0 if initial_data else 1
        SupplyItemsFormset = formset_factory(CustomerSupplyItemsForm, extra=extra_forms)
        customer_supply_items_formset = SupplyItemsFormset(initial=initial_data, prefix='customer_supply_items_formset',form_kwargs={'empty_permitted': False})
...
def edit_customer_supply(request, pk):
    ...
    salesman_instance = customer_supply_instance.salesman
    created_date = customer_supply_instance.created_date
    old_invoice_number = customer_supply_instance.invoice_no
    supply_items_instances = CustomerSupplyItems.objects.filter(
        customer_supply=customer_supply_instance
    )

    extra = 0 if supply_items_instances.exists() else 1
    ...
    if request.method == 'POST':
        ...
        customer_supply_form = EditCustomerSupplyForm(
            request.POST,
            instance=customer_supply_instance   # ✅ IMPORTANT
        )
        ...
        if customer_supply_form.is_valid() and customer_supply_items_formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    customer_suply_form_instance.created_by = customer_instance.sales_staff.pk
                    # customer_suply_form_instance.supply_date = created_date
                    # customer_suply_form_instance.created_date = customer_supply_instance.created_date
                    customer_suply_form_instance.invoice_no = old_invoice_number
                    customer_suply_form_instance.is_edited = True
                    customer_suply_form_instance.reference_number = "Supply edit from backend"
                    customer_suply_form_instance.save()
                    ...
                    CustomerSupplyItems.objects.filter(customer_supply=customer_supply_instance).delete()

                    for form in customer_supply_items_formset:
                        if form.cleaned_data:   # ✅ IMPORTANT LINE
                            item = form.save(commit=False)
                            item.customer_supply = customer_suply_form_instance
                            item.save()

                        if created_date.date() == datetime.today().date():
                            vanstock = VanProductStock.objects.get(created_date=created_date.date(), product=item.product, van=van)
                            
                            if vanstock.stock >= item.quantity:
                                
                                if customer_suply_form_instance.customer.sales_type != "FOC" :
                                    vanstock.sold_count += item.quantity
                                
                                if customer_suply_form_instance.customer.sales_type == "FOC" :
                                    vanstock.foc += item.quantity
                                    
                                if item.product.product_name == "5 Gallon" :
                                    total_fivegallon_qty += Decimal(item.quantity)
                                    vanstock.empty_can_count += customer_suply_form_instance.collected_empty_bottle
                                ...
                                response_data = {
                                    "status": "false",
                                    "title": "Failed",
                                    "message": f"No stock available in {item.product.product_name}, only {vanstock.stock} left",
                                }
                                return HttpResponse(json.dumps(response_data), content_type='application/javascript')    
                    ...
                    if customer_suply_form_instance.customer.sales_type != "FOC" :
                        ...
                        if old_invoice_number:
                            invoice = Invoice.objects.get(invoice_no=old_invoice_number)
                            invoice.created_date = invoice.created_date
                            invoice.invoice_date = created_date
                            invoice.net_taxable = customer_suply_form_instance.net_payable
                            invoice.vat = customer_suply_form_instance.vat
                            invoice.discount = customer_suply_form_instance.discount
                            invoice.amout_total = customer_suply_form_instance.subtotal
                            invoice.amout_recieved = customer_suply_form_instance.amount_recieved
                            
                            invoice.customer = customer_suply_form_instance.customer
                            if customer_suply_form_instance.amount_recieved == 0:
                            ...
                            if customer_suply_form_instance.amount_recieved == 0:
                                ...
                                inv_status = "paid"
                                inv_type = "cash_invoice"
                            # safety fallback only
                            invoice = Invoice.objects.create(
                                invoice_no=generate_invoice_no(),  # your generator
                                created_date=created_date,
                        ...
                        for item_data in supply_items:
                            ...
                            for item_data in supply_items:
                                ...
                                rate=item_data.amount,
                                invoice=invoice,
                                remarks='Invoice updated from edited supply'
                        )
                            # print("invoice generate")
                            InvoiceDailyCollection.objects.create(
                                invoice=invoice,
...
def custody_issue(request):
    report_type = request.GET.get('type', 'custody')  # custody | return
    ...
    if report_type == 'return':
        instances = CustomerReturn.objects.all().order_by('-created_date')
    else:
    ...
    if not start_date_str and not end_date_str:
        today = date.today()
        instances = instances.filter(created_date__date=today)
    ...
    if start_date_str and end_date_str:
        start_date = datetime.strptime(start_date_str, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d').date()
        instances = instances.filter(created_date__date__range=[start_date, end_date])

    # -----------------------------
    # ROUTE FILTER
    # -----------------------------
    if route_id:
        instances = instances.filter(customer__routes__route_id=route_id)
    ...
    for instance in instances:
        ...
        # -----------------------------
        if report_type == 'return':
            items = instance.customerreturnitems_set.all()
        else:
            items = instance.custodycustomitems_set.all()
        ...
        for item in items:
            ...
            if product.product_name == '5 Gallon':
                key = '5_gallon_deposit' if deposit_type == 'deposit' else '5_gallon_non_deposit'
                customer_product_counts[customer][key] += qty

            elif product.product_name == 'Dispenser':
                key = 'dispenser_deposit' if deposit_type == 'deposit' else 'dispenser_non_deposit'
                customer_product_counts[customer][key] += qty

            elif product.product_name == 'Cooler':
                key = 'cooler_deposit' if deposit_type == 'deposit' else 'cooler_non_deposit'
                customer_product_counts[customer][key] += qty

        # -----------------------------
        # SEND NOTIFICATION (ONLY FOR CUSTODY)
        # -----------------------------
        if report_type == 'custody' and customer.sales_staff:
            try:
                total_items = sum(
                    v for v in customer_product_counts[customer].values() if isinstance(v, int)
                )

                salesman_body = (
                    f'Customer custody issued for {customer}. '
                    f'Items issued: {total_items}, '
                    f'Agreement number: {instance.agreement_no}.'
                )

                notification(
                    customer.sales_staff.pk,
                    "Custody Issued",
                    salesman_body,
                    "Sanawaterfcm"
                )
                ...
                customer_body = (
                    f'Dear customer, custody items have been issued.\n'
                    f'Items: {total_items}\n'
                    f'Agreement No: {instance.agreement_no}'
                )
                ...
                messages.error(request, f'Notification error: {e}', 'alert-danger')
    ...
    customer_product_list = list(customer_product_counts.items())

    return render(
        request,
        'client_management/custody_issue.html',
        {
            'customer_product_counts': customer_product_list,
            'routes': routes,
...
#     # Route filter
#     if route_id:
#         instances = instances.filter(customer__routes_id=route_id)
...
#                   {'customer_product_counts': customer_product_list,'routes': routes,
#                     'selected_route': route_id,
#                     'start_date': start_date_str,
#                     'end_date': end_date_str,
#                     })

def print_custody_issue_view(request):
    report_type = request.GET.get('type', 'custody')  # custody | return
    ...
    # -----------------------------
    if report_type == 'return':
        instances = CustomerReturn.objects.all()
    else:
        instances = CustodyCustom.objects.all()
    ...
    if route_id:
        try:
            selected_route = RouteMaster.objects.get(id=route_id)
            instances = instances.filter(customer__routes=selected_route)
    ...
    for instance in instances:
        ...
        # ITEMS BASED ON REPORT TYPE
        if report_type == 'return':
            items = instance.customerreturnitems_set.all()
            deposit_type = 'non_deposit'  # returns don’t use deposit type
        else:
            items = instance.custodycustomitems_set.all()
            deposit_type = instance.deposit_type
        ...
        for item in items:
            ...
            if deposit_type == 'deposit':
                customer_product_counts[customer][f'{key}_deposit'] += qty
            else:
                customer_product_counts[customer][f'{key}_non_deposit'] += qty
    ...
    return render(
        request,
        'client_management/print_custody_issue.html',
        context
    )

def custody_issue_excel(request):
    data = get_custody_report_data(request)
    customer_product_counts = data['customer_product_counts']
    report_type = data['report_type']

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Custody Issue" if report_type == 'custody' else "Custody Return"

    ws.append([
        "Customer", "Route",
        "5G Deposit", "5G Non-Deposit",
        "Disp Deposit", "Disp Non-Deposit",
        "Cooler Deposit", "Cooler Non-Deposit"
    ])
    ...
    for customer, counts in customer_product_counts.items():
        ws.append([
            ...
            counts['5_gallon_deposit'],
            counts['5_gallon_non_deposit'],
            counts['dispenser_deposit'],
            counts['dispenser_non_deposit'],
            counts['cooler_deposit'],
            counts['cooler_non_deposit'],
        ])

    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    )
    filename = f"{report_type}_issue_report.xlsx"
...
@login_required
def custody_item_return(request, customer_id):
    customer = get_object_or_404(Customers, customer_id=customer_id)

    custody_stocks = CustomerCustodyStock.objects.filter(customer=customer)

    if request.method == "POST":
        try:
            custody_stock_id = request.POST.get('custody_stock_id')
            ...
            custody_return = CustomerReturn.objects.create(
                customer=customer,
            ...
            # Create Return Items
            item = CustomerReturnItems.objects.create(
                customer_return=custody_return,
                product=product,
                quantity=quantity,
            ...
            # Update Custody Stock
            custody_stock = CustomerCustodyStock.objects.get(id=custody_stock_id)
            custody_stock.quantity -= quantity
            custody_stock.amount_collected -= total_amount
            custody_stock.save()
            ...
            if product.product_name == "5 Gallon":
                bottle_count = BottleCount.objects.filter(
                    van__salesman=request.user,
                    created_date__date=custody_return.created_date.date()
                ).first()
                if bottle_count:
                    bottle_count.custody_issue += quantity
                    bottle_count.save()

            messages.success(request, "Custody item returned successfully")
            return redirect('custody_item_return', customer_id=customer.customer_id)

        except Exception as e:
            messages.error(request, str(e))

    return render(request, 'client_management/custody_item_return.html', {
        'customer': customer,
        'custody_stocks': custody_stocks
    })
...
def customer_outstanding_list(request):
    ...
    # -------------------- SALESMAN LIST --------------------
    salesman_list = CustomUser.objects.filter(user_type="Salesman").order_by('first_name', 'last_name')
    ...
    if salesman_id:
        ...
        print("outstanding_qs:",outstanding_qs.values())
        print("==================================")
    ...
    if salesman_id:
        ...
        print("collection_qs:",collection_qs.values())
    ...
    user_permissions = []
    if hasattr(request.user, 'designation_id') and request.user.designation_id:
        user_permissions = request.user.designation_id.allowed_tabs or []
...
def export_to_excel(instances, date, total_amount, total_bottles, total_coupons, product_type):
    ...
    headers = ["Sl No", "Customer ID", "Customer Name", "Building No", "Room No", "Route", "Outstanding Amount", "Bottles", "Coupons"]
    ...
    # Total Row
    sheet[f"F{row_no}"] = "Total"
    sheet[f"G{row_no}"] = round(total_amount, 2)
    sheet[f"H{row_no}"] = total_bottles
    sheet[f"I{row_no}"] = total_coupons
...
def customer_outstanding_details(request, customer_pk):
    ...
    context = {
        ...
        "user_permissions": list(request.user.designation_id.allowed_tabs) if hasattr(request.user, 'designation_id') and request.user.designation_id else [],
    }
...
def delete_invoice_outstanding(request, invoice_no, customer_pk):
    """
    Delete an invoice outstanding (positive outstanding)
    """
    try:
        with transaction.atomic():
            ...
            # 2️⃣ Get OutstandingAmount related to this invoice
            ...
            if customer_outstanding:
                outstanding = OutstandingAmount.objects.select_for_update().filter(
                    customer_outstanding=customer_outstanding
                ).first()
                if outstanding:
            ...
            # If not found in CustomerOutstanding, but invoice exists
            if not outstanding_amount:
                outstanding_amount = invoice.amout_total - invoice.amout_recieved

            # 3️⃣ Update CustomerOutstandingReport
            report = CustomerOutstandingReport.objects.select_for_update().filter(
                customer=customer,
                product_type="amount"
            ).first()
            ...
            return JsonResponse({
                ...
                "message": "Invoice and outstanding deleted successfully",
                "reload": "true",
            })
...
def customer_count_excel(request):
    routes = RouteMaster.objects.all()

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Customer Count"

    # Header row
    ws.append([
        "Sl No", "Route Name", "Sales Man",
        "Cash", "Credit", "Coupon", "FOC", "Total"
    ])

    sl_no = 1
    total_cash = total_credit = total_coupon = total_foc = total_customers = 0

    for route in routes:
        van_route = Van_Routes.objects.filter(routes=route).first()
        sales_man = van_route.van.salesman if van_route and van_route.van else ''

        cash = Customers.objects.filter(
            is_guest=False, is_deleted=False, routes=route, sales_type='CASH'
        ).count()

        credit = Customers.objects.filter(
            is_guest=False, is_deleted=False, routes=route, sales_type='CREDIT'
        ).count()

        coupon = Customers.objects.filter(
            is_guest=False, is_deleted=False, routes=route,
            sales_type__in=['CASH COUPON', 'CREDIT COUPON']
        ).count()

        foc = Customers.objects.filter(
            is_guest=False, is_deleted=False, routes=route, sales_type='FOC'
        ).count()

        total = cash + credit + coupon + foc
        if total == 0:
            continue

        ws.append([sl_no, route.route_name, str(sales_man), cash, credit, coupon, foc, total])
    ...
    # Total row
    ws.append(["", "", "TOTAL", total_cash, total_credit, total_coupon, total_foc, total_customers])

    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    )
    response['Content-Disposition'] = 'attachment; filename=route_wise_customer_count.xlsx'

    wb.save(response)
    return response


def print_customer_count_view(request):
    routes = RouteMaster.objects.all()
    ...
    for route in routes:
        ...
        route_total = cash + credit + coupon + foc
        if route_total == 0:
            continue
    ...
    return render(request, 'client_management/print_customer_count.html', context)
...
def ageing_report_view(request): 
    ...
    salesman_id = request.GET.get('salesman_id', None)
    salesman_list = CustomUser.objects.filter(user_type="Salesman").order_by('first_name', 'last_name')
    ...
    if selected_route:
        ageing_data = get_customer_outstanding_aging(selected_route.route_name, salesman_id)
    ...
    context = {
        ...
        'totals': totals,
        'salesman_list': salesman_list,
        'selected_salesman_id': salesman_id,
    }
...
def print_ageing_report_view(request):
    ...
    context = {
        'selected_route': selected_route,
        'routes': routes,
        'selected_salesman_id': salesman_id if salesman_id else '',  # Ensure it is not None string
        'totals': totals,
    }
...
def ageing_report_excel(request):
    ...
    # Fix: Pass route_name string, not object
    route_arg = selected_route.route_name if selected_route else None
    aging_report = get_customer_outstanding_aging(route_arg, salesman_id)
    ...
    if not df.empty:
        ...
        numeric_cols = ['less_than_30', 'between_31_and_60', 'between_61_and_90', 
                       'between_91_and_150', 'between_151_and_365', 'more_than_365', 'grand_total']
        
        for col in numeric_cols:
            if col in df.columns:
                totals[col] = df[col].astype(float).sum()
        
        totals['customer_name'] = 'Total'
...
def export_customer_outstanding_to_excel(request):
    ...
    for item in instances:
        ...
        data.append([
            ...
            item.customer.door_house_no,
            item.customer.routes.route_name if item.customer.routes else "",
            item.get_outstanding_count()
        ])
    
    # Create a DataFrame
    df = pd.DataFrame(data, columns=['Created Date', 'Invoice No', 'Customer', 'Building No', 'Room No', 'Route', 'Count'])
...
class AuditDetailsView(View):
    def get(self, request, customer_id):
        ...
        if not audit_base:
            audit_base = AuditBase.objects.create(
                route=customer.routes,
                start_date=timezone.now()
            )

        audit_details = AuditDetails.objects.filter(
...
def customer_outstanding_report_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    from django.db.models import Sum
    from django.http import HttpResponse
    
    # Imports assumed available
    ...
    # Default dates logic strictly following view logic
    closing_date = to_date = datetime.today().date()
    ...
    outstanding_customer_ids = []
    if route_id:
        # outstanding_customer_ids = CustomerOutstanding.objects.filter(customer__routes__pk=route_id,created_date__date__lt=closing_date,product_type='amount').values_list("customer__pk", flat=True).distinct()
        # The view logic seems to fetch ids based on outstanding existence but then filters customers by ID.
        # Actually, let's just use the same logic as the view to get the customer list
         outstanding_customer_ids = CustomerOutstanding.objects.filter(customer__routes__pk=route_id,created_date__date__lt=closing_date,product_type='amount').values_list("customer__pk", flat=True).distinct()
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Customer Outstanding Report"
    ...
    headers = [
        ...
        f'Collection Amount Up to {to_date.strftime("%d-%m-%Y")}', 
        'Balance Amount'
    ]
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
    ...
    row_num = 2
    for idx, instance in enumerate(customer_instances, start=1):
        customer_pk = instance.pk
        
        # Logic from get_customer_wise_outstanding_details template tag
        opening_amount = OutstandingAmount.objects.filter(
            customer_outstanding__customer__pk=customer_pk, 
            customer_outstanding__created_date__date__lte=closing_date
        ).aggregate(total_amount=Sum('amount'))['total_amount'] or 0
        
        opening_collection = CollectionPayment.objects.filter(
            customer__pk=customer_pk, 
            created_date__date__lte=closing_date
        ).aggregate(total_amount=Sum('amount_received'))['total_amount'] or 0
        
        opening_outstanding = opening_amount - opening_collection
        
        collection_upto = CollectionPayment.objects.filter(
            customer__pk=customer_pk, 
            created_date__date__gt=closing_date, 
            created_date__date__lte=to_date
        ).aggregate(total_amount=Sum('amount_received'))['total_amount'] or 0
        
        balance_amount = opening_outstanding - collection_upto

        row = [
            idx,
            instance.custom_id,
            instance.customer_name,
            instance.building_name,
            instance.door_house_no,
            opening_outstanding,
            collection_upto,
            balance_amount
        ]
    ...
    # Totals Row (optional but good for consistency with report)
    # The view calculates global totals, but iterating in Excel is easier.
    # Let's add a total row at the bottom
    last_row = row_num - 1
    if last_row >= 2:
        sheet.cell(row=row_num, column=3, value="Total").font =  workbook.active.cell(row=row_num, column=3).font.copy(bold=True)
        sheet.cell(row=row_num, column=6, value=f"=SUM(F2:F{last_row})").font = workbook.active.cell(row=row_num, column=6).font.copy(bold=True)
        sheet.cell(row=row_num, column=7, value=f"=SUM(G2:G{last_row})").font = workbook.active.cell(row=row_num, column=7).font.copy(bold=True)
        sheet.cell(row=row_num, column=8, value=f"=SUM(H2:H{last_row})").font = workbook.active.cell(row=row_num, column=8).font.copy(bold=True)


    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Customer_Outstanding_Report_{closing_date}_to_{to_date}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)

    return response

def customer_transaction_list_excel(request):
    from openpyxl import Workbook
    ...
    # Imports assumed available in views.py or imported here
    from client_management.models import CustomerOutstanding, CustomerSupplyItems, CustomerSupply, CustomerCouponItems
    from sales_management.models import CollectionPayment
    from accounts.models import Customers
    ...
    # -----------------------------
    # 1. FETCH DATA (Same logic as view)
    # -----------------------------
    
    # Get CustomerSupply items
    sales = CustomerSupplyItems.objects.filter(
        customer_supply__customer__pk=customer_pk
    ).exclude(customer_supply__customer__sales_type__in=["CASH COUPON", "CREDIT COUPON"]).order_by('-customer_supply__supply_date')
    ...
    if start_date_str and end_date_str:
        try:
            ...
            sales = sales.filter(customer_supply__supply_date__date__range=(start_date, end_date))
            coupons = coupons.filter(customer_coupon__created_date__date__range=(start_date, end_date))
            customer_outstanding_instances = customer_outstanding_instances.filter(outstanding_date__date__range=(start_date, end_date))
            collection_payment_instance = collection_payment_instance.filter(created_date__date__range=(start_date, end_date))
            redeemed_coupon_instances = redeemed_coupon_instances.filter(created_date__date__range=(start_date, end_date))
    ...
    sheet_sales.title = "Sales Report"
    
    headers_sales = ['Sl No', 'Supply Date', 'Reference No', 'Invoice No', 'Product Name', 'Sales Type', 'QTY', 'Amount', 'Discount', 'Net Taxable', 'VAT', 'Grand Total', 'Collected']
    for col_num, header in enumerate(headers_sales, start=1):
        cell = sheet_sales.cell(row=1, column=col_num, value=header)
        cell.font = Font(bold=True)
    ...
    for sale in sales:
        sheet_sales.append([
            sl_no,
            sale.customer_supply.supply_date.date() if sale.customer_supply.supply_date else "",
            sale.customer_supply.reference_number,
            sale.customer_supply.invoice_no,
            sale.product.product_name,
            sale.customer_supply.customer.sales_type,
            sale.quantity,
            sale.customer_supply.grand_total,
            sale.customer_supply.discount,
            sale.customer_supply.subtotal,
            sale.customer_supply.vat,
            sale.customer_supply.grand_total,
            sale.customer_supply.amount_recieved
        ])
    ...
    # Coupon Data in Sales Sheet
    for coupon in coupons:
        vat_rate = Tax.objects.get(name="VAT").percentage if Tax.objects.filter(name="VAT").exists() else 0
        sheet_sales.append([
            sl_no,
            coupon.customer_coupon.created_date.date() if coupon.customer_coupon.created_date else "",
            coupon.customer_coupon.reference_number,
            coupon.customer_coupon.invoice_no,
            coupon.coupon.book_num,
            coupon.customer_coupon.customer.sales_type,
            1,
            coupon.customer_coupon.grand_total,
            coupon.customer_coupon.discount,
            coupon.customer_coupon.net_amount,
            vat_rate,
            coupon.customer_coupon.grand_total,
            coupon.customer_coupon.amount_recieved
        ])
    ...
    # Totals Row
    sheet_sales.append(["Total", "", "", "", "", "", total_qty, total_amount, total_discount, total_net_payable, total_vat, total_grand_total, total_amount_recieved])
    for col in range(1, 14):
        sheet_sales.cell(row=row_num, column=col).font = Font(bold=True)

    # --- SHEET 2: Customer Outstanding ---
    sheet_outstanding = workbook.create_sheet("Customer Outstanding")
    headers_outstanding = ['Sl No', 'Date', 'Product Type', 'Invoice No', 'Outstanding Amount', 'Outstanding Coupons', 'Empty Cans']
    for col_num, header in enumerate(headers_outstanding, start=1):
        cell = sheet_outstanding.cell(row=1, column=col_num, value=header)
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal='center')
    ...
    total_out_amount = 0
    total_out_coupons = 0
    total_out_emptycan = 0

    for outstanding in customer_outstanding_instances:
        out_amount = outstanding.outstandingamount_set.aggregate(total=Sum('amount'))['total'] or 0
        out_coupons = outstanding.outstandingcoupon_set.aggregate(total=Sum('count'))['total'] or 0
        out_emptycan = outstanding.outstandingproduct_set.aggregate(total=Sum('empty_bottle'))['total'] or 0
        
        sheet_outstanding.append([
            sl_no,
            outstanding.outstanding_date.date() if outstanding.outstanding_date else "",
            outstanding.product_type,
            outstanding.invoice_no,
            out_amount,
            out_coupons,
            out_emptycan
        ])
        ...
        total_out_coupons += out_coupons
        total_out_emptycan += out_emptycan
        sl_no += 1
        row_num += 1

    sheet_outstanding.append(["Total", "", "", "", total_out_amount, total_out_coupons, total_out_emptycan])
    for col in range(1, 8):
        sheet_outstanding.cell(row=row_num, column=col).font = Font(bold=True)

    # --- SHEET 3: Customer Collection Report ---
    sheet_collection = workbook.create_sheet("Collection Report")
    headers_collection = ['Sl No', 'Created Date', 'Payment Method', 'Receipt No', 'Amount Received', 'Total Discounts', 'Net Taxable', 'VAT', 'Collected']
    for col_num, header in enumerate(headers_collection, start=1):
        cell = sheet_collection.cell(row=1, column=col_num, value=header)
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal='center')
    ...
    total_col_received = 0
    total_col_discount = 0
    total_col_net = 0
    total_col_vat = 0
    total_col_collected = 0

    for collection in collection_payment_instance:
        t_disc = collection.total_discounts()
        t_net = collection.total_net_taxeble()
        t_vat = collection.total_vat()
        t_col = collection.collected_amount()
        
        sheet_collection.append([
            sl_no,
            collection.created_date.date() if collection.created_date else "",
        ...
        total_col_received += collection.amount_received or 0
        total_col_discount += t_disc or 0
        total_col_net += t_net or 0
        total_col_vat += t_vat or 0
        total_col_collected += t_col or 0
        
        sl_no += 1
        row_num += 1

    sheet_collection.append(["Total", "", "", "", total_col_received, total_col_discount, total_col_net, total_col_vat, total_col_collected])
    for col in range(1, 10):
        sheet_collection.cell(row=row_num, column=col).font = Font(bold=True)
        
    # --- SHEET 4: Redeemed Coupons ---
    sheet_redeemed = workbook.create_sheet("Redeemed Coupons")
    headers_redeemed = ['Sl No', 'Supply Date', 'Invoice No', 'Manual Coupons', 'Digital Coupons']
    for col_num, header in enumerate(headers_redeemed, start=1):
        cell = sheet_redeemed.cell(row=1, column=col_num, value=header)
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal='center')
    ...
    for redeemed in redeemed_coupon_instances:
        coupons_dict = redeemed.total_coupon_recieved()
        manual = coupons_dict.get('manual_coupon', 0)
        digital = coupons_dict.get('digital_coupon', 0)
        
        sheet_redeemed.append([
            sl_no,
            redeemed.created_date.date() if redeemed.created_date else "",
            redeemed.invoice_no,
            manual,
            digital
        ])
    ...
    sheet_redeemed.append(["Total", "", "", total_manual, total_digital])
    for col in range(1, 6):
        sheet_redeemed.cell(row=row_num, column=col).font = Font(bold=True)

    # -----------------------------
    # 3. RESPONSE
    # -----------------------------
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Customer_Transaction_List_{customer_pk}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
