...
class Invoice(models.Model):
    ...
    def sub_total(self):
        """Sum of customer_rate * qty for all items"""
        total = 0
        items = InvoiceItems.objects.filter(invoice=self)
        for item in items:
            total += item.customer_rate * item.qty
        return total
...
class InvoiceItems(models.Model):
    ...
    def customer_rate(self):
        """
        Returns the correct customer-specific rate for this product:
        1. Customer's custom rate (Customers.rate) if set
        2. Product_Default_Price_Level rate by customer_type + product
        3. ProdutItemMaster.rate (product default)
        4. Stored item.rate as final fallback
        """
        ...
        try:
            ...
            if customer.rate and Decimal(str(customer.rate)) > 0:
                return Decimal(str(customer.rate))
            # Priority 2: price level by customer_type + product
            if self.product_items and customer.customer_type:
                try:
                    price_level = Product_Default_Price_Level.objects.get(
                        product_id=self.product_items,
                        customer_type=customer.customer_type
                    )
                    return Decimal(str(price_level.rate))
                except Product_Default_Price_Level.DoesNotExist:
                    pass
            # Priority 3: product master rate
            if self.product_items and self.product_items.rate and self.product_items.rate > 0:
                return self.product_items.rate
        except Exception:
            pass
        # Priority 4: stored item rate
        return self.rate

    @property
    def line_total(self):
        """Calculated total: customer_rate x qty"""
        return self.customer_rate * self.qty
