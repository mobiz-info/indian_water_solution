...
def invoice_list(request):
    """
    Invoice List with date range and route filter
    """
    ...
    if start_date:
        try:
            ...
            instances = instances.filter(invoice_date__gte=start)
    ...
    if end_date:
        try:
            ...
            instances = instances.filter(invoice_date__lte=end)
...
def today_route_outstanding_list(request):
    ...
    route_li = RouteMaster.objects.all()
    salesman_li = CustomUser.objects.filter(
    ...
    for customer in customers:
        ...
        # 🔍 SALESMAN FILTER
        if salesman_id:
            invoice_qs = invoice_qs.filter(salesman__id=salesman_id)
...
@login_required
def invoices(request,invoice_no):
    """
    Invoice Download
    :param request:
    :return: Invoices Download view
    ...
    instance = Invoice.objects.get(invoice_no=invoice_no,is_deleted=False)  
    # log_activity(
    #     created_by=request.user,
    #     description=f"Viewed invoice with ID {pk}."
    # )       
