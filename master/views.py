...
class Designation_Create(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            data = form.save(commit=False)
            data.created_by = str(request.user.id)
            data.allowed_tabs = form.cleaned_data.get("allowed_tabs", [])
            data.save()
...
def subscription_alert(request):
    """
    Renders the subscription alert screen when the subscription is true / locked.
    """
    try:
        from master.models import SubscriptionSettings
        settings = SubscriptionSettings.objects.first()
        if not settings or not settings.is_subscription_active:
            return redirect('dashboard')
    except Exception:
        pass
        
    return render(request, 'master/subscription_alert.html')

from django.contrib.auth.decorators import login_required
from django.contrib import messages

@login_required
def app_permissions_list(request):
    try:
        from master.models import AppPermission, RouteMaster
        perm, created = AppPermission.objects.get_or_create(permission_name="Customer Rate Edit")
    except Exception:
        perm = None
    ...
    return render(request, 'master/app_permissions.html', context)

@login_required
def update_app_permissions(request):
    if request.method == "POST":
        try:
            from master.models import AppPermission
            perm, created = AppPermission.objects.get_or_create(permission_name="Customer Rate Edit")
            is_active = request.POST.get('is_active') == 'on'
            route_ids = request.POST.getlist('routes')
            
            perm.is_active = is_active
            perm.routes.set(route_ids)
            perm.save()
