...
def migrate_salesman(
    ...
    print(f"From         : {old_salesman_id}")
    print(f"To           : {new_salesman_id}")
    print(f"Before Date  : {end_date}")
    print("-" * 70)
    ...
    outstanding_qs = CustomerOutstanding.objects.filter(
        created_by=str(old_salesman_id),
        outstanding_date__lt=end_date,
        customer__routes__route_name=route_name
    )
    ...
    print(f"DailyCollection        : {daily_collection_qs.count()}")
    print(f"Outstanding            : {outstanding_qs.count()}")
    ...
    with transaction.atomic():
        ...
        print("Supply updated          :", supply_qs.update(salesman_id=new_salesman_id))
        print("Collection updated      :", collection_qs.update(salesman_id=new_salesman_id))
        print("DailyCollection updated :", daily_collection_qs.update(salesman_id=new_salesman_id))
        print("Outstanding updated     :", outstanding_qs.update(created_by=str(new_salesman_id)))

        print("\n✅ DONE")

migrate_salesman(
    old_salesman_id=821,
    new_salesman_id=2658,
    route_name="S-16",
    end_date=datetime(2026, 4, 1, 0, 0, 0),  # April 1
    do_update=True
)

# outstanding_qs = CustomerOutstanding.objects.filter(
#     created_by = str(471),
#     created_date__gte=datetime(2026, 1, 1, 23, 59, 59), 
# )
# print("Total records:", outstanding_qs.count())

# for o in outstanding_qs.select_related('customer', 'customer__routes',):
#     customer_name = o.customer.customer_name if o.customer else "N/A"
#     customer_id = o.customer.custom_id if o.customer else "N/A"
#     route_name = o.customer.routes.route_name if o.customer and o.customer.routes else "N/A"
#     created_date = o.created_date

#     print(
#         # f"Outstanding ID: {o.id} | "
