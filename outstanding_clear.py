...
# excel_path = "S-36 ROUTE OUTSTANDING UPTO 14-FEB-26 (1).xlsx"
# wb = openpyxl.load_workbook(excel_path)
# sheet = wb.active
...
# for row in sheet.iter_rows(min_row=2, values_only=True):
#     customer_id = str(row[4])   # column 2 = Customer ID
#     outstanding_val = row[7] 
...
def clear_multiple_invoices_s36():

    # 🔹 Get all customers in S-36 route
    customers = Customers.objects.filter(
        routes__route_name="S-36"   # adjust field if needed
    )
    ...
    for customer in customers:
        ...
        invoices = Invoice.objects.filter(
            customer=customer,
            is_deleted=False
        ).order_by("-created_date")  # latest first
        ...
        # All others
        old_invoices = invoices.exclude(id=keep_invoice.id)

        with transaction.atomic():
            for inv in old_invoices:
                mark_invoice_paid(inv)
                total_invoices_cleared += 1

        total_customers_processed += 1

        print(
            f"✔ Customer {customer.custom_id} | "
            f"Kept Invoice {keep_invoice.invoice_no} | "
            f"Cleared {old_invoices.count()} invoices"
        )

    print("\n------ SUMMARY ------")
    print("Customers processed:", total_customers_processed)
    print("Invoices cleared   :", total_invoices_cleared)


# 🔥 RUN
clear_multiple_invoices_s36()
