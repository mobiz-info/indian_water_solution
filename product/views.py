...
class Products_List(View):
    ...
    @method_decorator(login_required)
    def get(self, request, *args, **kwargs):
        all_products = Product.objects.select_related('product_name', 'product_name__category', 'branch_id').order_by('-created_date')
        merged_instances = []
        seen = set()
        
        for p in all_products:
            key = (p.product_name_id, p.branch_id_id)
            if key not in seen:
                seen.add(key)
                merged_instances.append(p)
            else:
                for m in merged_instances:
                    if m.product_name_id == p.product_name_id and m.branch_id_id == p.branch_id_id:
                        m_qty = m.quantity if m.quantity else 0
                        p_qty = p.quantity if p.quantity else 0
                        m.quantity = m_qty + p_qty
                        break
                        
        context = {'instances': merged_instances}
        return render(request, self.template_name, context)
