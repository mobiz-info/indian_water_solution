...
class CollectionPaymentAdmin(admin.ModelAdmin):
    ...
    list_filter = ('payment_method','created_date', 'customer__routes')

    autocomplete_fields = ['salesman', 'customer',]

# Admin for CollectionItems
class CollectionItemsAdmin(admin.ModelAdmin):
    list_display = ['invoice_number','collection_date', 'amount', 'balance', 'amount_received', 'collection_payment']

    def invoice_number(self, obj):
        return obj.invoice.invoice_number
    invoice_number.short_description = "Invoice Number"

    def collection_date(self, obj):
        return obj.collection_payment.created_date
    collection_date.short_description = "Date"
