...
class PendingChequeCollection(models.Model):
    """
    Stores cheque collection records submitted from the mobile sales app.
    No CollectionPayment is created at this point — the actual payment
    is only processed when the admin clears the cheque on the web.
    """
    customer = models.ForeignKey('accounts.Customers', on_delete=models.CASCADE)
    salesman = models.ForeignKey('accounts.CustomUser', on_delete=models.CASCADE, null=True, blank=True)
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    def __str__(self):
        return f"Pending Cheque - {self.customer} - {self.cheque_no} ({self.status})"
