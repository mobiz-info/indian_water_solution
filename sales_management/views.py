...
from django.db.models import Q, ExpressionWrapper, Subquery, OuterRef
...
def product_route_salesreport(request):
    ...
    product_id = request.GET.get("selected_product_id")
    ...
    # --------------------------------------------------------------
    # 1️⃣ SUPPLY SIDE (CustomerSupply + CustomerSupplyItems)
    # --------------------------------------------------------------
    supplies = CustomerSupply.objects.all()

    if start_date and end_date:
        supplies = supplies.filter(created_date__date__range=[start_date, end_date])

    if route_name:
        supplies = supplies.filter(customer__routes__route_name=route_name)

    if product_id:
        supplies = supplies.filter(customersupplyitems__product_id=product_id)

    supply_report = supplies.values(
        "customer__routes__route_name",
    ).annotate(
        cash_qty=Sum(
            "customersupplyitems__quantity",
            filter=Q(amount_recieved__gt=0)
        ),

        credit_qty=Sum(
            "customersupplyitems__quantity",
            filter=Q(amount_recieved=0) &
                   ~Q(customer__sales_type="CASH COUPON")
        ),

        coupon_qty=Sum(
            "customersupplyitems__quantity",
            filter=Q(customer__sales_type="CASH COUPON")
        ),

        foc_qty=Sum("customersupplyitems__foc"),

        total_qty=Sum("customersupplyitems__quantity"),
    )

   
    custody = CustodyCustomItems.objects.select_related(
        "custody_custom", "custody_custom__customer", "product"
    )

    if start_date and end_date:
        custody = custody.filter(custody_custom__created_date__date__range=[start_date, end_date])

    if route_name:
        custody = custody.filter(custody_custom__customer__routes__route_name=route_name)

    if product_id:
        custody = custody.filter(product_id=product_id)

    custody_report = custody.values(
        "custody_custom__customer__routes__route_name",
    ).annotate(
        cash_qty=Sum(
            "quantity",
            filter=Q(custody_custom__amount_collected__gt=0)
        ),

        credit_qty=Sum(
            "quantity",
            filter=Q(custody_custom__amount_collected=0) &
                   ~Q(custody_custom__sales_type="CASH COUPON")
        ),

        coupon_qty=Sum(
            "quantity",
            filter=Q(custody_custom__sales_type="CASH COUPON")
        ),

        foc_qty=Sum(
            "quantity",
            filter=Q(custody_custom__sales_type="FOC")
        ),
    ...
    # Merge supply
    for row in supply_report:
        r = row["customer__routes__route_name"]
        final[r]["route"] = r
        final[r]["cash_qty"] += row["cash_qty"] or 0
        final[r]["credit_qty"] += row["credit_qty"] or 0
        final[r]["coupon_qty"] += row["coupon_qty"] or 0
        final[r]["foc_qty"] += row["foc_qty"] or 0
        final[r]["total_qty"] += row["total_qty"] or 0

    # Merge custody
    for row in custody_report:
        r = row["custody_custom__customer__routes__route_name"]
        final[r]["route"] = r
        final[r]["cash_qty"] += row["cash_qty"] or 0
        final[r]["credit_qty"] += row["credit_qty"] or 0
        final[r]["coupon_qty"] += row["coupon_qty"] or 0
    ...
    final_report = list(final.values())

    # --------------------------------------------------------------
    # 4️⃣ SUMMARY TOTALS
    # --------------------------------------------------------------
    summary = {
        "cash_total": sum(r["cash_qty"] for r in final_report),
        "credit_total": sum(r["credit_qty"] for r in final_report),
        "coupon_total": sum(r["coupon_qty"] for r in final_report),
        "foc_total": sum(r["foc_qty"] for r in final_report),
        "total_qty": sum(r["total_qty"] for r in final_report),
    }
    ...
    return render(request, "sales_management/product_route_salesreport.html", context)

def product_route_salesreport_detail(request):

    # ---- parse filters exactly like your working report ----
    ...
    supply = supply_items.values("customer_supply__created_date__date").annotate(
        cash_qty=Sum("quantity", filter=Q(customer_supply__amount_recieved__gt=0)),
        credit_qty=Sum("quantity", filter=Q(customer_supply__amount_recieved=0) &
                                    ~Q(customer_supply__customer__sales_type="CASH COUPON")),
        coupon_qty=Sum("quantity", filter=Q(customer_supply__customer__sales_type="CASH COUPON")),
        foc_qty=Sum("foc"),
        total_qty=Sum("quantity")
    )

    # ------- CUSTODY -------
    custody_items = CustodyCustomItems.objects.filter(
        product_id=product_id,
        custody_custom__customer__routes__route_name=route_name,
        custody_custom__created_date__date__range=(start_date, end_date)
    )

    custody = custody_items.values("custody_custom__created_date__date").annotate(
        cash_qty=Sum("quantity", filter=Q(custody_custom__amount_collected__gt=0)),
        credit_qty=Sum("quantity", filter=Q(custody_custom__amount_collected=0) &
                                    ~Q(custody_custom__sales_type="CASH COUPON")),
        coupon_qty=Sum("quantity", filter=Q(custody_custom__sales_type="CASH COUPON")),
        foc_qty=Sum("quantity", filter=Q(custody_custom__sales_type="FOC")),
        total_qty=Sum("quantity")
    )
    ...
    for row in supply:
        d = row["customer_supply__created_date__date"]
        f = final[d]
        f["date"] = d
        f["cash_qty"] += row["cash_qty"] or 0
        f["credit_qty"] += row["credit_qty"] or 0
        f["coupon_qty"] += row["coupon_qty"] or 0
        f["foc_qty"] += row["foc_qty"] or 0
        f["total_qty"] += row["total_qty"] or 0

    for row in custody:
        d = row["custody_custom__created_date__date"]
        f = final[d]
        f["date"] = d
        f["cash_qty"] += row["cash_qty"] or 0
        f["credit_qty"] += row["credit_qty"] or 0
    ...
    rows = sorted(final.values(), key=lambda x: x["date"])

    # summary
    summary = {
        "cash_total": sum(r["cash_qty"] for r in rows),
        "credit_total": sum(r["credit_qty"] for r in rows),
        "coupon_total": sum(r["coupon_qty"] for r in rows),
        "foc_total": sum(r["foc_qty"] for r in rows),
        "total_qty": sum(r["total_qty"] for r in rows)
    }

    print("DEBUG route_name:", route_name)
    print("DEBUG product_id:", product_id)
    print("SUPPLY COUNT:", supply_items.count())
    print("CUSTODY COUNT:", custody_items.count())

    return render(request, "sales_management/product_route_salesreport_details.html", {
        "rows": rows,
        "summary": summary,
        "start_date": start_date,
        "end_date": end_date,
...
def product_route_salesreport_invoice_detail(request):

    date_raw = request.GET.get("date")
    route_name = request.GET.get("route_name")
    product_id = request.GET.get("selected_product_id")

    target_date = datetime.datetime.strptime(date_raw, "%Y-%m-%d").date()

    # SUPPLY ITEMS
    supply_items = CustomerSupplyItems.objects.filter(
        product_id=product_id,
        customer_supply__customer__routes__route_name=route_name,
        customer_supply__created_date__date=target_date
    ).select_related("customer_supply", "customer_supply__customer")

    # CUSTODY ITEMS
    custody_items = CustodyCustomItems.objects.filter(
        product_id=product_id,
        custody_custom__customer__routes__route_name=route_name,
        custody_custom__created_date__date=target_date
    ).select_related("custody_custom", "custody_custom__customer")
    ...
    for item in supply_items:
        ...
        cs = item.customer_supply

        # Determine invoice type
        if cs.amount_recieved > 0:
            invoice_type = "Cash Invoice"
        elif cs.amount_recieved == 0 and cs.customer.sales_type == "CASH COUPON":
            invoice_type = "Coupon Invoice"
        else:
            invoice_type = "Credit Invoice"

        final_rows.append({
            "customer_name": cs.customer.customer_name,
            "customer_id": cs.customer.custom_id,
            "invoice_no": cs.invoice_no,
    ...
    for item in custody_items:
        ...
        ct = item.custody_custom

        # Determine invoice type (using custody fields)
        if ct.amount_collected > 0:
            invoice_type = "Cash Invoice"
        elif ct.amount_collected == 0 and ct.sales_type == "CASH COUPON":
            invoice_type = "Coupon Invoice"
        else:
            invoice_type = "Credit Invoice"

        final_rows.append({
            "customer_name": ct.customer.customer_name,
            "customer_id": ct.customer.custom_id,
            "invoice_no": ct.invoice_no,
            "invoice_type": invoice_type,
            "qty": qty,
        })

    return render(request, "sales_management/product_route_invoice_details.html", {
        "rows": final_rows,
        "total_qty": total_qty,
        "date": target_date,
        "route_name": route_name
...
def customerSales_report(request):
    ...
    sales = CustomerSupply.objects.select_related('customer', 'salesman').filter(
        supply_date__date__gte=start_date,
        supply_date__date__lte=end_date
    ...
    if route_name:
        ...
        if van_route:
            ...
            cash_sales = CustomerSupply.objects.filter(
                supply_date__date__range=[date_from, date_to],
                salesman=salesman,
                amount_recieved__gt=0
            ).exclude(customer__sales_type="CASH COUPON").order_by('supply_date')
...
def creditsales_report(request):
    ...
    if route_name:
        ...
        ### credit sales ####
        credit_sales = CustomerSupply.objects.filter(supply_date__date__range=[from_date, to_date],salesman=salesman,amount_recieved__lte=0).exclude(customer__sales_type__in=["FOC","CASH COUPON"]).order_by('created_date')
        ...
        recharge_credit_sales = CustomerCoupon.objects.filter(created_date__date__range=[from_date, to_date],salesman=salesman,amount_recieved__lte=0).order_by('created_date')
...
def dsr_coupon_sales(request):
    ...
    if route_name:
        ...
        if van_route:
            ...
            coupon_sales = CustomerSupply.objects.filter(
                ...
                route_name=Subquery(
                    Van_Routes.objects.filter(van__salesman=OuterRef('salesman')).values('routes__route_name')[:1]
                )
    ...
    manual_coupon_total = CustomerSupplyCoupon.objects.filter(customer_supply__in=coupon_sales).aggregate(Count('leaf'))['leaf__count'] or 0
...
def delete_coupon_sale(request, sale_id):
    import json
    from invoice_management.models import Invoice
    from django.db import transaction as db_transaction
    try:
        with db_transaction.atomic():
            supply = CustomerSupply.objects.get(id=sale_id)
            invoice_no = supply.invoice_no
            supply.delete()
            if invoice_no:
                Invoice.objects.filter(invoice_no=invoice_no).delete()

        response_data = {
            "status": "true",
            "title": "Successfully Deleted",
            "message": "Coupon sale successfully deleted.",
            "redirect": "true",
            "redirect_url": reverse('dsr_coupon_sales'),
        }
    except CustomerSupply.DoesNotExist:
        response_data = {
            "status": "false",
            "title": "Failed",
            "message": "Coupon sale not found.",
        }
    except Exception as e:
        response_data = {
            "status": "false",
...
def coupon_book_sale_delete(request, pk):
    """Delete a CustomerCoupon sale and restore coupon books to rechargeable status,
    restore VanCouponStock, and decrease CustomerCouponStock leaflet count."""
    from client_management.models import CustomerCoupon, CustomerCouponItems, CustomerCouponStock
    from coupon_management.models import NewCoupon
    from van_management.models import VanCouponStock
    if request.method == 'POST':
        try:
            with transaction.atomic():
                customer_coupon = CustomerCoupon.objects.get(id=pk)
                items = CustomerCouponItems.objects.filter(customer_coupon=customer_coupon)

                for item in items:
                    new_coupon = item.coupon

                    # 1. Restore the coupon book to rechargeable (False = available)
                    new_coupon.status = False
                    new_coupon.save()

                    # 2. Restore VanCouponStock: add back 1 to stock, decrease sold_count
                    # VanCouponStock has one record per date, so use filter+first to get the latest
                    try:
                        van_coupon_stock = VanCouponStock.objects.filter(coupon=new_coupon).order_by('-created_date').first()
                        if van_coupon_stock:
                            van_coupon_stock.stock += 1
                            if van_coupon_stock.sold_count > 0:
                                van_coupon_stock.sold_count -= 1
                            van_coupon_stock.save()
                    except Exception:
                        pass  # Skip silently if any issue

                    # 3. Decrease CustomerCouponStock leaflet count (for customers_coupon API)
                    try:
                        coupon_stock = CustomerCouponStock.objects.get(
                            customer=customer_coupon.customer,
                            coupon_type_id=new_coupon.coupon_type
                        )
                        leaflets = int(new_coupon.coupon_type.no_of_leaflets)
                        coupon_stock.count = max(0, coupon_stock.count - leaflets)
                        coupon_stock.save()
                    except CustomerCouponStock.DoesNotExist:
                        pass  # No customer stock record found, skip silently

                customer_coupon.delete()
                return JsonResponse({'success': True, 'message': 'Coupon book sale deleted and coupon books restored.'})
...
def coupon_book_sale_change_date(request, pk):
    """Change the date (created_date) of a CustomerCoupon sale."""
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            original_time = customer_coupon.created_date.time() if customer_coupon.created_date else dt.time(0, 0)
            customer_coupon.created_date = dt.datetime.combine(new_date.date(), original_time)
            customer_coupon.save()
            return JsonResponse({'success': True, 'message': 'Date updated successfully.', 'new_date': new_date_str})
        except CustomerCoupon.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'Record not found.'}, status=404)
        except ValueError:
...
def five_gallon_rates_report(request):
    """
    5 Gallon Supplied Rates Report — date range + route filter.
    Mirrors get_five_gallon_ratewise_count logic exactly:
    - unique rates from customer__rate field (same as original DSR)
    - filters using calculated amount/quantity rounded to 2dp
    """
    ...
    route_names   = request.GET.getlist('route_name')
    ...
    if not request.GET:
        ...
        return render(request, 'sales_management/five_gallon_rates_report.html', context)

    data_filter = True

    # Base queryset: 5 Gallon supply items for date range
    base_qs = CustomerSupplyItems.objects.filter(
        customer_supply__supply_date__date__gte=from_date,
        customer_supply__supply_date__date__lte=to_date,
        product__product_name="5 Gallon",
        quantity__gt=0,
        amount__gt=0,
    )
    ...
    if route_names:
        filter_data['route_name'] = route_names
        # Filter if 'all' is not part of the selection
        if "all" not in [str(r).lower() for r in route_names]:
            base_qs = base_qs.filter(customer_supply__customer__routes__route_name__in=route_names)

    base_qs = base_qs.distinct()

    unique_rates = sorted(set(
        base_qs.values_list('customer_supply__customer__rate', flat=True)
    ))

    # Step 2: For each rate, count credit/cash/coupon using the same
    # calculated_rate=amount/quantity logic as get_five_gallon_ratewise_count
    for rate in unique_rates:
        rate_qs = base_qs.annotate(
        ...
        ).filter(calculated_rate=rate)

        cash_qty = (
            rate_qs.filter(customer_supply__amount_recieved__gt=0)
                   .aggregate(total=Sum("quantity"))["total"] or 0
        )
        credit_qty = (
            rate_qs.filter(customer_supply__amount_recieved=0)
                   .exclude(customer_supply__customer__sales_type__in=["FOC", "CASH COUPON"])
                   .aggregate(total=Sum("quantity"))["total"] or 0
        )
        coupon_qty = (
            rate_qs.filter(customer_supply__customer__sales_type="CASH COUPON")
                   .aggregate(total=Sum("quantity"))["total"] or 0
        )
        rates_data.append({
            'rate': rate,
            ...
            'cash': cash_qty,
            'coupon': coupon_qty,
            'total': credit_qty + cash_qty + coupon_qty,
        })
    ...
    context = {
        ...
        'total_credit': sum(r['credit'] for r in rates_data),
        'total_cash':   sum(r['cash']   for r in rates_data),
        'total_coupon': sum(r['coupon'] for r in rates_data),
        'grand_total':  sum(r['total']  for r in rates_data),
    }
    return render(request, 'sales_management/five_gallon_rates_report.html', context)


def five_gallon_rates_report_print(request):
    """
    Print view for 5 Gallon Supplied Rates Report.
    """
    from django.db.models import DecimalField, ExpressionWrapper, F, Sum
    from django.db.models.functions import Round
    ...
    return render(request, 'sales_management/five_gallon_rates_report_print.html', context)


def five_gallon_rates_report_excel(request):
    """
    Excel export view for 5 Gallon Supplied Rates Report.
    """
    from django.db.models import DecimalField, ExpressionWrapper, F, Sum
    from django.db.models.functions import Round
    import openpyxl
    ...
    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    worksheet.title = '5 Gallon Supplied Rates'
    
    # Metadata
    worksheet.append(['5 Gallon Supplied Rates Report'])
    worksheet.append(['Route:', route_name])
    worksheet.append(['Date Range:', f"{from_date.strftime('%d-%m-%Y')} to {to_date.strftime('%d-%m-%Y')}"])
    worksheet.append([])

    headers = ['Rate (AED)', 'Credit (Bottles)', 'Cash (Bottles)', 'Coupon (Bottles)', 'Total']
    worksheet.append(headers)
    ...
    worksheet.append(['Total', total_credit, total_cash, total_coupon, grand_total])
    ...
    for col_num in range(1, 6):
        worksheet.cell(row=5, column=col_num).font = Font(bold=True) # Headers
        worksheet.cell(row=worksheet.max_row, column=col_num).font = Font(bold=True) # Footer
    ...
    response["Content-Disposition"] = f'attachment; filename="5_Gallon_Rates_Report_{from_date}_{to_date}.xlsx"'
    return response
...
def dsr_summary(request):
    ...
    if date:
        date = datetime.datetime.strptime(date, '%Y-%m-%d').date()
        filter_data['filter_date'] = date.strftime('%Y-%m-%d')
        filter_data['display_date'] = date.strftime('%d-%m-%Y')
    else:
        date = datetime.datetime.today().date()
        filter_data['filter_date'] = date.strftime('%Y-%m-%d')
        filter_data['display_date'] = date.strftime('%d-%m-%Y')
    ...
    if route_name:
        ...
        five_gallon_cash_total_net_taxable = five_gallon_cash_sales.aggregate(total_net_taxable=Sum('amount_before_vat'))['total_net_taxable'] or 0
        five_gallon_cash_total_vat = five_gallon_cash_sales.aggregate(total_vat=Sum('vat_amount'))['total_vat'] or 0
        ...
        other_cash_total_net_taxable = other_cash_sales.aggregate(total_net_taxable=Sum('amount_before_vat'))['total_net_taxable'] or 0
        other_cash_total_vat = other_cash_sales.aggregate(total_vat=Sum('vat_amount'))['total_vat'] or 0
        ...
        five_gallon_credit_total_net_taxable = five_gallon_credit_sales.aggregate(total_net_taxable=Sum('amount_before_vat'))['total_net_taxable'] or 0
        five_gallon_credit_total_vat = five_gallon_credit_sales.aggregate(total_vat=Sum('vat_amount'))['total_vat'] or 0
        ...
        other_credit_total_net_taxable = other_credit_sales.aggregate(total_net_taxable=Sum('amount_before_vat'))['total_net_taxable'] or 0
        other_credit_total_vat = other_credit_sales.aggregate(total_vat=Sum('vat_amount'))['total_vat'] or 0
    ...
    context = {
        ...
        'has_cgst_sgst': Tax.objects.filter(name__icontains='cgst').exists() or Tax.objects.filter(name__icontains='sgst').exists(),
    }
...
def collection_list_view(request):
    ...
    total_collected = instances.aggregate(total=Sum('amount_received'))['total'] or 0
...
def collection_list_excel(request):
    import openpyxl
    ...
    ws = wb.active
    ws.title = "Collection List"

    header_font = Font(bold=True, color="FFFFFF")
    header_fill = PatternFill(start_color="2563EB", end_color="2563EB", fill_type="solid")
    center = Alignment(horizontal="center", vertical="center")

    headers = ["#", "Date", "Customer Name", "Building Name", "Customer ID", "Salesman", "Route", "Payment Method", "Receipt Number", "Total Amount", "Collected Amount"]
    ...
    for row_num, instance in enumerate(instances, 2):
        created_date = instance.created_date.strftime('%d-%m-%Y %H:%M') if instance.created_date else ''
        collected = instance.amount_received or 0
        total_collected += float(collected)
        ws.append([
            row_num - 1,
            created_date,
            str(instance.customer) if instance.customer else '',
            instance.customer.building_name if instance.customer else '',
            instance.customer.custom_id if instance.customer else '',
            str(instance.salesman) if instance.salesman else '',
            instance.customer.routes.route_name if instance.customer and instance.customer.routes else '',
            instance.payment_method or '',
            instance.receipt_number or '',
    ...
    total_cell = ws.cell(row=total_row, column=10, value="Total Collected:")
    total_cell.font = Font(bold=True)
    total_cell.alignment = Alignment(horizontal="right")
    total_val = ws.cell(row=total_row, column=11, value=total_collected)
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Collection_List_{start_date or 'all'}_to_{end_date or 'all'}.xlsx"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    wb.save(response)
    return response
...
def delete_collection_payment(request, pk):
    ...
    for collection_payment in collection_payments:
        ...
        for item in collection_items:
            ...
            item.delete()
        log_activity(request.user.id, f"Deleted collection payment: {collection_payment.receipt_number}, {collection_payment.customer.customer_name} for amount {collection_payment.amount_received}")
        
        # Delete the associated Receipt
        # Receipt.objects.filter(receipt_number=collection_payment.receipt_number).delete()
...
def monthly_sales_report(request):
    ...
    monthly_sales = (
        CustomerSupply.objects
        .filter(supply_date__year=year, supply_date__month=month)
    ...
    for sale in monthly_sales:
        ...
        total_supply_qty = CustomerSupplyItems.objects.filter(
            customer_supply__salesman_id=salesman_id,
            customer_supply__supply_date__year=year,
            customer_supply__supply_date__month=month
...
def _build_routewise_report_context(request):
...
def routewise_sales_report(request, route_id):
    ...
    context = _build_routewise_report_context(request)
    context.update({
        'route': route,
        'van_route': van_route,
    ...
    return render(request, 'sales_management/routewise_sales_report.html', context)
...
def print_routewise_sales_report(request, route_id):
    ...
    return render(request, 'sales_management/print_routewise_sales_report.html', context)
...
def cheque_collections_view(request):
    from sales_management.models import PendingChequeCollection
    start_date = request.GET.get("start_date")
    end_date = request.GET.get("end_date")
    route_name = request.GET.get("route_name")
    ...
    # ── Pending cheques from mobile app (no CollectionPayment yet) ──
    pending_qs = PendingChequeCollection.objects.filter(status="PENDING").select_related('customer', 'customer__routes', 'salesman')
    if start_date:
        pending_qs = pending_qs.filter(created_date__gte=start_date)
    if end_date:
        pending_qs = pending_qs.filter(created_date__lte=end_date)
    if route_name:
        pending_qs = pending_qs.filter(customer__routes__route_name__iexact=route_name)
    if search_query:
    ...
    for p in pending_qs:
        invoice_numbers = [inv.invoice_no for inv in p.invoices.all()]
        collection_list.append({
            "collection_id": p.id,
            "is_pending": True,          # flag so the template can show correct clearance URL
            "customer": p.customer.customer_name,
            "route": p.customer.routes.route_name if p.customer.routes else "No Route",
            "custom_id": p.customer.custom_id,
            "payment_method": "Cheque",
    ...
    # ── Already-cleared cheques (CollectionPayment records) ──
    collections = CollectionPayment.objects.filter(payment_method__iexact="cheque")
    if start_date:
    ...
    for collection in collections:
        invoice_numbers = [item.invoice.invoice_no for item in collection.collectionitems_set.all()]
...
def cheque_clearance(request, collection_id, is_pending=False):
    """Clear a cheque — works for both PendingChequeCollection (mobile) and
    legacy CollectionPayment-based cheques (web)."""
    from sales_management.models import PendingChequeCollection
    try:
        with transaction.atomic():

            # ── Determine source: pending (mobile) or already-created payment ──
            pending_obj = PendingChequeCollection.objects.filter(pk=collection_id, status="PENDING").first()

            if pending_obj:
                # Mobile-submitted cheque: create CollectionPayment NOW
                customer = pending_obj.customer
                amount_received = pending_obj.amount
                invoices = pending_obj.invoices.all().order_by("created_date")

                collection_payment = CollectionPayment.objects.create(
                    payment_method="CHEQUE",
                    customer=customer,
                    salesman=pending_obj.salesman,
                    amount_received=amount_received,
                )
                CollectionCheque.objects.create(
                    collection_payment=collection_payment,
                    cheque_amount=amount_received,
                    cheque_no=pending_obj.cheque_no,
                    bank_name=pending_obj.bank_name,
                    cheque_date=pending_obj.cheque_date,
                    status="CLEARED",
                )
            else:
                # Legacy web-submitted cheque already has a CollectionPayment
                collection_payment = get_object_or_404(CollectionPayment, pk=collection_id)
                customer = collection_payment.customer
                amount_received = collection_payment.amount_received
                collection_cheque = CollectionCheque.objects.filter(collection_payment=collection_payment).first()
                if collection_cheque and collection_cheque.invoices.exists():
                    invoices = collection_cheque.invoices.all().order_by("created_date")
                else:
                    invoices = Invoice.objects.filter(
                        collectionitems__collection_payment=collection_payment
            ...
            for invoice in invoices:
                if invoice.amout_total > invoice.amout_recieved and remaining_amount > 0:
                    ...
                    CollectionItems.objects.create(
                        ...
                        balance=due_amount - payment_amount,
            ...
            CollectionCheque.objects.filter(collection_payment=collection_payment).update(status="CLEARED")
...
# def collection_payment(request, customer_id):
#     customer = get_object_or_404(Customers, pk=customer_id)
#     salesman = Van_Routes.objects.filter(
#         routes=customer.routes,
#         van__salesman__designation_id__designation_name__in=["Sales Executive", "Salesman"]
#     ).latest("created_date").van.salesman

#     invoices = Invoice.objects.filter(
#         customer=customer, invoice_status="non_paid", is_deleted=False
#     ).exclude(amout_total=0)

#     total_amount = sum(inv.amout_total - inv.amout_recieved for inv in invoices)
...
#             payment_method = request.POST.get("payment_method")
#             amount_received = Decimal(request.POST.get("amount_received", 0))
#             invoice_ids = request.POST.getlist("invoice_ids") or [inv.id for inv in invoices]
...
#             invoices_to_pay = Invoice.objects.filter(pk__in=invoice_ids, customer=customer).order_by("created_date")
...
#             payment_date_str = request.POST.get("payment_date")
#             payment_date = parse_payment_date(payment_date_str)
...
#             # Create receipt first (will update invoice_number later)
#             receipt = Receipt.objects.create(
#                 transaction_type="collection",
#                 instance_id=str(collection_payment.id),
#                 amount_received=amount_received,
#                 customer=customer,
#                 invoice_number="",
#                 created_date=collection_payment.created_date
#             )
#             collection_payment.receipt_number = receipt.receipt_number
#             collection_payment.save()
...
#                     collection_payment=collection_payment,
#                 )
#                 collection_cheque.invoices.set(invoices_to_pay)  
#                 print("linked invoices:", list(collection_cheque.invoices.all()))
...
#                     log_activity(
#                         created_by=request.user,
#                         description=f"Invoice {invoice.invoice_no} paid: {payment_amount} received, Remaining balance: {invoice.amout_total - invoice.amout_recieved}"
#                     )

#                 if remaining_amount <= 0:
#                     break

#             # update receipt with invoices
#             receipt.invoice_number = ",".join(invoice_numbers)
#             receipt.save()

#             # Refund logic (if extra amount left)
#             if remaining_amount > 0:
...
#                 "status": "true",
#                 "title": "Success",
#                 "message": "Collection payment saved successfully.",
#                 "redirect": "true",
#                 "redirect_url": reverse("customers"),
#             })
...
def cashsales_report_excel(request):
    ...
    if route_name:
        ...
        if van_route:
            ...
            cash_total_net_taxable += cash_sale_recharge_net_payeble
            cash_total_vat += cash_sale_recharge_vat_total
            cash_total_subtotal += cash_sale_recharge_grand_total
            cash_total_amount_recieved += cash_sale_recharge_amount_recieved
            cash_total_amount_recieved = cash_total_received + cash_sale_recharge_amount_recieved
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Cash Sales Report"

    # Write Headers
    headers = ['Sl No', 'Date', 'Invoice No', 'Customer Id', 'Customer Name', 'Total QTY', 'Rate', 'Grand Total', 'Net Payable', 'Cash Recieved']
    ...
    for sale in cash_sales:
        supply_date_naive = sale.supply_date
        if isinstance(supply_date_naive, datetime) and supply_date_naive.tzinfo is not None:
                supply_date_naive = supply_date_naive.astimezone(timezone.utc).replace(tzinfo=None)
        
        # Format date to dd-mm-yyyy
        formatted_date = supply_date_naive.strftime('%d-%m-%Y') if supply_date_naive else ""
        ...
        sheet.append([
            ...
            sale.customer.custom_id,
            sale.customer.customer_name,
            sale.get_total_supply_qty(),
            sale.customer.get_water_rate(),
            sale.subtotal,
            sale.net_payable,
            sale.amount_recieved
        ])
    ...
    # Process Recharge Cash Sales
    for sale in recharge_cash_sales:
        created_date_naive = sale.created_date
        if isinstance(created_date_naive, datetime) and created_date_naive.tzinfo is not None:
                created_date_naive = created_date_naive.astimezone(timezone.utc).replace(tzinfo=None)
    ...
    sheet.append([
        ...
        "-",
        cash_total_subtotal,
        cash_total_net_taxable,
        cash_total_amount_recieved
    ])

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Cash_Sales_Report_{date_from.strftime('%d-%m-%Y')}_to_{date_to.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
...
def creditsales_report_excel(request):
    ...
    if route_name:
        ...
        if van_route:
            ...
            credit_sales = CustomerSupply.objects.filter(
                supply_date__date__range=[from_date, to_date],
                salesman=salesman,
                amount_recieved__lte=0
            ).exclude(customer__sales_type__in=["FOC","CASH COUPON"]).order_by('created_date')
            ...
            credit_total_net_taxable += credit_sale_recharge_net_payeble 
            credit_total_vat += credit_sale_recharge_vat_total 
            credit_total_subtotal += credit_sale_recharge_grand_total
            credit_total_amount_recieved += credit_total_received + credit_sale_recharge_amount_recieved
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Credit Sales Report"
    ...
    for sale in credit_sales:
        ...
        # Format date to dd-mm-yyyy
        formatted_date = created_date_naive.strftime('%d-%m-%Y') if created_date_naive else ""
    ...
    sheet.append([
        ...
        credit_total_qty,
        "-",
        credit_total_subtotal,
        credit_total_net_taxable,
        credit_total_amount_recieved
    ])

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Credit_Sales_Report_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
...
def total_sales_report(request):
    ...
    van_route = Van_Routes.objects.none
    salesman_id = ""
    sales = CustomerSupply.objects.none()
    recharge_sales = CustomerCoupon.objects.none()
    products = ProdutItemMaster.objects.none
    routes_instances = RouteMaster.objects.all()
    van_product_stock = VanProductStock.objects.none
    ...
    date_to_str = request.GET.get('date_to')
    route_name = request.GET.get('route_name')
    q = request.GET.get('q') # Search query (customer name or ID)
    
    if date_from_str and date_to_str:
        date_from = datetime.datetime.strptime(date_from_str, '%Y-%m-%d').date()
        date_to = datetime.datetime.strptime(date_to_str, '%Y-%m-%d').date()
        filter_data['date_from'] = date_from.strftime('%Y-%m-%d')
        filter_data['date_to'] = date_to.strftime('%Y-%m-%d')
        data_filter = True
    else:
        date_from = datetime.datetime.today().date()
        date_to = date_from
    ...
    if route_name:
        ...
        if van_route:
            ...
            # Stock report (optional, copied from cash sales if needed, but maybe just sales logic for now)
            
            # 1. Total Customer Supply (Sales)
            # Exclude FOC but include CASH COUPON
            sales = CustomerSupply.objects.filter(
                supply_date__date__range=[date_from, date_to],
                salesman=salesman
            ).exclude(customer__sales_type="FOC").order_by('supply_date')
            
            if q:
                filter_data['q'] = q
                sales = sales.filter(
                    Q(customer__customer_name__icontains=q) | 
                    Q(customer__custom_id__icontains=q)
                )
            
            total_net_taxable = sales.aggregate(total_net_taxable=Sum('net_payable'))['total_net_taxable'] or 0
            total_vat = sales.aggregate(total_vat=Sum('vat'))['total_vat'] or 0
            total_subtotal = sales.aggregate(total_subtotal=Sum('subtotal'))['total_subtotal'] or 0
            total_received = sales.aggregate(total_amount_recieved=Sum('amount_recieved'))['total_amount_recieved'] or 0
            total_quantity = sales.aggregate(total_quantity=Sum('customersupplyitems__quantity'))['total_quantity'] or 0
            
            # Removed Coupon Recharge section as per request
            recharge_sales = []
            
            total_amount_recieved = total_received 
    ...
    context = {
        ...
        'total_vat': total_total_vat if 'total_total_vat' in locals() else total_vat, # check logic
        'total_vat': total_vat,
    ...
    return render(request, 'sales_management/dsr_total_sales_report.html', context)


def total_sales_report_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    from django.utils import timezone
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Total Sales Report"
    
    # Write Headers
    headers = ['Sl No', 'Date', 'Invoice No', 'Customer Id', 'Customer Name', 'Sales Type', 'Total QTY', 'Rate', 'Grand Total', 'Net Payable', 'Received Amount']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
    ...
    for sale in sales:
        ...
        sales_type = "Cash"
        if sale.customer.sales_type == "CASH COUPON":
            sales_type = "Coupon"
        elif sale.amount_recieved == 0:
            sales_type = "Credit"
    ...
    # Removed Recharge Sales Loop as per request
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Total_Sales_Report_{date_from.strftime('%d-%m-%Y')}_to_{date_to.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_coupon_sales_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Coupon Sales Report"
    
    # Headers
    headers = ['Sl No', 'Reference No', 'Customer Id', 'Customer Name', 'Total QTY', 'Rate', 'Manual Coupon Recieved', 'Digital Coupon Recieved']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for index, sale in enumerate(coupon_sales, start=1):
        sheet.append([
            index,
            sale.reference_number,
            sale.customer.custom_id,
            ...
            sale.get_total_supply_qty(),
            sale.customer.get_water_rate(),
            sale.total_coupon_recieved()['manual_coupon'],
            sale.total_coupon_recieved()['digital_coupon']
        ])
        row_num += 1
    ...
    sheet.append([
        ...
        "-",
        manual_coupon_total,
        digital_coupon_total
    ])
    
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Coupon_Sales_Report_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
...
def dsr_coupon_book_sales_excel(request):
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Coupon Book Sales"
    
    # Headers
    headers = ['Sl No', 'Date', 'Ref No', 'Customer Name', 'Customer Id', 'Coupon Type', 'Book No', 'Method', 'Rate', 'Total', 'Discount', 'Total Payable', 'Amount Received', 'Balance']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for index, coupon in enumerate(customer_coupons, start=1):
        # Flatten details for simplicity (or we can add multiple rows if needed, but flattening is safer for simple report)
        coupon_items = CustomerCouponItems.objects.filter(customer_coupon=coupon)
        
        # Taking the first item or comma separating values if multiple (assuming logic handles single main item usually, or adjust)
        # For Excel, let's list them. If multiple, we might want to just list them in one cell or duplicate rows.
        # Let's concatenate for now as it's cleaner for a single row per transaction.
        ...
        for item in coupon_items:
            c_types.append(item.coupon.coupon_type.coupon_type_name)
            book_nums.append(str(item.coupon.book_num))
            methods.append(item.coupon.coupon_method)
            rates.append(str(item.rate))
            
        c_type_str = ", ".join(c_types)
        book_num_str = ", ".join(book_nums)
        method_str = ", ".join(methods)
        rate_str = ", ".join(rates)
        
        created_date_naive = coupon.created_date.replace(tzinfo=None) if coupon.created_date else ""
        
        sheet.append([
            index,
            created_date_naive.strftime('%d-%m-%Y') if created_date_naive else "",
            coupon.reference_number,
            coupon.customer.customer_name,
            coupon.customer.custom_id,
            c_type_str,
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Coupon_Book_Sales_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_foc_customers_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "FOC Customers Report"
    
    # Headers
    headers = ['Sl No', 'Customer ID', 'Customer Name', 'Route', 'Quantity']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for index, foc in enumerate(foc_customers, start=1):
        sheet.append([
            index,
            foc.customer.custom_id,
            foc.customer.customer_name,
            foc.customer.routes.route_name if foc.customer.routes else "",
            foc.get_total_supply_qty()
        ])
        row_num += 1
        
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"FOC_Customers_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_stock_report_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    # Headers
    headers = ['Date', 'Product Name', 'Opening Stock', 'Issued', 'Sold', 'FOC', 'Returned', 'Damage Bottle', 'Empty Bottle', 'Fresh Stock', 'Closing']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for data in van_product_stock:
        # Replicating logic from get_van_product_wise_stock tag for Issued Count
        # Tag logic: issued_count = staff_order_details.aggregate(total_count=Sum('issued_qty'))
        # We need to replicate this per row (per date/van/product)
        # Note: data is a VanProductStock instance
        
        issued_count = 0
        if data.van and data.van.salesman:
             issued_count = Staff_Orders_details.objects.filter(
                staff_order_id__order_date=data.created_date,
                product_id=data.product,
                staff_order_id__created_by=data.van.salesman.pk
            ).aggregate(total_count=Sum('issued_qty'))['total_count'] or 0
        ...
        sheet.append([
            ...
            data.return_count,
            data.damage_count,
            data.empty_can_count,
            stock_report_total, # As per template behavior
            data.closing_count
        ])
        row_num += 1
        
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Stock_Report_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_expense_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    # Headers
    headers = ['Sl No', 'Expense Head', 'Amount']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for index, expense in enumerate(expenses_instanses, start=1):
        sheet.append([
            index,
            expense.expence_type.name,
            expense.amount
        ])
        row_num += 1
        
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Expense_Report_{date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_five_gallon_rates_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    from django.db.models import Sum, F, DecimalField, ExpressionWrapper
    from django.db.models.functions import Round
    
    five_gallon_rates = set()
    salesman_id = None
    ...
    if route_name:
        ...
        if van_route:
            salesman = van_route.van.salesman
            salesman_id = salesman.pk
            five_gallon_rates = set(CustomerSupplyItems.objects.filter(
                customer_supply__created_date__date=date,
                customer_supply__salesman_id=salesman,
                product__product_name="5 Gallon"
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "5 Gallon Rates"
    
    # Headers
    headers = ['Rate', 'Credit', 'Cash', 'Coupon']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for rate in five_gallon_rates:
        # Replicating logic from get_five_gallon_ratewise_count
        ...
        debit_amount_count = qs.filter(customer_supply__amount_recieved__gt=0).aggregate(total=Sum("quantity"))["total"] or 0
        credit_amount_count = qs.filter(customer_supply__amount_recieved=0).exclude(customer_supply__customer__sales_type__in=["FOC", "CASH COUPON"]).aggregate(total=Sum("quantity"))["total"] or 0
        coupon_amount_count = qs.filter(customer_supply__customer__sales_type="CASH COUPON").aggregate(total=Sum("quantity"))["total"] or 0
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"5_Gallon_Rates_{date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def dsr_credit_outstanding_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    if route_name:
        ...
        if van_route:
            ...
            # collection details
            dialy_collections = CollectionPayment.objects.filter(salesman_id=salesman.pk,amount_received__gt=0) 
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Credit Outstanding"
    
    # Headers
    headers = ['Credit Outstanding', 'Credit Outstanding Collection', 'Net Outstanding']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    # Data Row
    sheet.append([
        outstanding_credit_notes_total_amount,
        outstanding_credit_notes_received_amount,
        outstanding_credit_notes_balance
    ])
        
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Credit_Outstanding_{date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
...
def visitstatistics_report_excel(request):
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Visit Statistics"
    
    # Headers
    headers = ['Route', 'Salesman', 'Driver', 'New Customers Created', 'Planned Visit', 'Actual Visit', 'Non Visited', 'Emergency Supply']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
    ...
    for route in routes_instances:
        ...
        if van_route:
            ...
            # Find today's customers (We need to replicate find_customers logic or just use route customer count if close enough? 
            # The original view calls `find_customers`. Inspecting that might be complex. 
            # Let's see if we can import it. It is likely in `sales_management.views` or imported there.
            # Assuming `find_customers` is available in views scope. 
            # If not, I might need to redefine it or assume it's imported in `views.py`. 
            # Since I am appending to views.py, it should be available if it's defined in views.py or imported there.
            # I saw `todays_customers = find_customers(request, str(from_date), route.pk)` in view.
            ...
            sheet.append([
                ...
                new_customers_count,
                planned_visit_count,
                visited_customers_count,
                non_visited_count,
                emergency_supply_count
            ])
            row_num += 1

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Visit_Statistics_{from_date.strftime('%d-%m-%Y')}_to_{to_date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def outstanding_amount_collected_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    ...
    if route_name:
        ...
        if van_route:
            salesman = van_route.van.salesman
            dialy_collections = CollectionPayment.objects.filter(salesman_id=salesman.pk, amount_received__gt=0, created_date__date=date)

    # Create Workbook
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Collection Report"
    
    # Headers
    headers = ['Sl.no', 'Customer Name', 'Building Name/Room no', 'Customer Type', 'Amount Collected', 'Mode']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
        
    row_num = 2
    for index, instance in enumerate(dialy_collections, start=1):
        building_info = instance.customer.building_name
        if instance.customer.door_house_no:
            building_info += f" / {instance.customer.door_house_no}"
            
        sheet.append([
            index,
            instance.customer.customer_name,
            building_info,
            instance.customer.customer_type,
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Outstanding_Collected_{date.strftime('%d-%m-%Y')}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def monthly_sales_report_excel(request):
    from openpyxl import Workbook
    from datetime import datetime
    import calendar
    ...
    sheet = workbook.active
    month_name = calendar.month_name[month]
    sheet.title = f"Monthly Sales - {month_name}"
    
    # Headers
    headers = ['Salesman', 'Grand Total', 'Discount', 'Net Payable', 'VAT', 'Subtotal', 'Amount Received', 'Supply Quantity', 'Coupons Received (Manual)', 'Coupons Received (Digital)']
    for col_num, header in enumerate(headers, start=1):
        sheet.cell(row=1, column=col_num, value=header)
    ...
    for sale in monthly_sales:
        ...
        sheet.append([
            sale['salesman__username'],
            sale['total_grand_total'],
            sale['total_discount'],
            sale['total_net_payable'],
            sale['total_vat'],
            sale['total_subtotal'],
            sale['total_amount_received'],
            total_supply_qty,
            manual_coupon_count,
            digital_coupon_count
        ])
        row_num += 1
        
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Monthly_Sales_Report_{month_name}_{year}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
    
    return response

def detailed_sales_report_excel(request):
    from openpyxl import Workbook
    ...
    # Imports assumed to be available in views.py context or added here if needed
    # But since this is a snippet to be appended, I'll assume standard imports are present 
    # or import distinct models inside if they are not common. 
    # To be safe, re-importing models used in the logic.
    from sales_management.models import CollectionPayment
    from van_management.models import Expense
    from client_management.models import CustomerSupply, CustomerCoupon, CustomerCouponItems
    from master.models import RouteMaster

    current_date = datetime.now()
    default_from_date = current_date.replace(day=1)
    default_to_date = (default_from_date + timedelta(days=1)) # Just a default, logic below handles parsing
    ...
    try:
        if from_date_str:
            start_date = make_aware(datetime.strptime(from_date_str, "%Y-%m-%d"))
        else:
            start_date = make_aware(default_from_date)
            
        if to_date_str:
            end_date = make_aware(datetime.strptime(to_date_str, "%Y-%m-%d"))
        else:
            end_date = make_aware(default_to_date)
    except ValueError:
        start_date, end_date = make_aware(default_from_date), make_aware(default_to_date)
    ...
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Detailed Sales Report"
    ...
    for route in report_routes:
        ...
        # Logic from get_sales_report template tag
        supplies = CustomerSupply.objects.filter(customer__routes_id=route_id, supply_date__date__range=(start_date, end_date))
    ...
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    filename = f"Detailed_Sales_Report_{start_date.date()}_to_{end_date.date()}.xlsx"
    response['Content-Disposition'] = f'attachment; filename={filename}'
    workbook.save(response)
...
def monthly_outstanding_report(request):
    ...
    # Initialize variables related to the report
    daily_data = []
    total_credit = 0
    total_collection = 0
    total_closing = 0
    ...
    routes_instances = RouteMaster.objects.all()
    # Fetch all salesmen. Assuming they are CustomUser or similar. 
    # Based on other views, often 'salesman' is a CustomUser.
    # Let's try to find how to get all salesmen. 
    # Usually: CustomUser.objects.filter(user_type='Salesman')
    # Use a safe fallback or broad query if unsure, but strict is better.
    # I'll check imports in previous step output.
    salesmen_instances = CustomUser.objects.filter(user_type='Salesman').order_by('first_name', 'last_name')

    # Get filter parameters
    from_date_str = request.GET.get('from_date')
    to_date_str = request.GET.get('to_date')
    route_name = request.GET.get('route_name')
    salesman_param = request.GET.get('salesman_id')
    ...
    if from_date_str and to_date_str:
        ...
        filter_data['from_date'] = from_date_str
        filter_data['to_date'] = to_date_str
    else:
        # Default to current month
        today = datetime.datetime.today().date()
        from_date = today.replace(day=1)
        # Last day of current month logic
    ...
    if route_name or salesman_param:
        data_filter = True
        
        # Filter Logic
        # If route is selected, use that.
        # If salesman is selected, we need to find their routes or filter by customer__sales_staff?
        # Requirement says "Salesman Filter". 
        # Existing logic uses `van_route` to find customers via `customer__routes`.
        # If salesman is selected, we can try to find the route assigned to this salesman.
        # Or filter customers by salesman directly if applicable.
        ...
        if route_name:
            filter_data['route_name'] = route_name
            # Get specific route
            # Note: RouteMaster is the model for route_name (likely).
            # Van_Routes maps Van <-> Route.
            van_route_obj = Van_Routes.objects.filter(routes__route_name=route_name).first()
            if van_route_obj:
                target_routes = [van_route_obj.routes]
                salesman = van_route_obj.van.salesman
                salesman_id = salesman.pk
                salesman_name = salesman.get_fullname()
                filter_data['salesman_id'] = str(salesman.pk)
        
        elif salesman_param:
            filter_data['salesman_id'] = salesman_param
            salesman_id = salesman_param
            salesman = CustomUser.objects.get(pk=salesman_param)
            salesman_name = salesman.get_fullname()
            
            # Find routes for this salesman
            # Van has salesman. Van_Routes has van and routes.
            van_routes = Van_Routes.objects.filter(van__salesman=salesman)
            target_routes = [vr.routes for vr in van_routes]
            
        
        if target_routes:
            # We can aggregate across multiple routes if salesman has multiple,
            # but usually 1 salesman -> 1 van -> 1 route (or list of routes).
            # Logic below assumes single iteration or aggregation.
            # Let's aggregate.
            
            # --- 1. Calculate Opening Balance (Pre-Period) ---
            pre_period_credit = OutstandingAmount.objects.filter(
                customer_outstanding__created_date__date__lt=from_date,
                customer_outstanding__product_type="amount",
                customer_outstanding__customer__routes__in=target_routes
            ).aggregate(total=Sum('amount'))['total'] or 0
            
            pre_period_collection = CollectionPayment.objects.filter(
                created_date__date__lt=from_date,
                customer__routes__in=target_routes
            ).aggregate(total=Sum('amount_received'))['total'] or 0

            opening_outstanding = pre_period_credit - pre_period_collection
            
            # --- 2. Iterate Days ---
            current_date = from_date
            running_balance = opening_outstanding
            
            while current_date <= to_date:
                # Daily Credit Sales
                daily_credit = OutstandingAmount.objects.filter(
                    customer_outstanding__created_date__date=current_date,
                    customer_outstanding__product_type="amount",
                    customer_outstanding__customer__routes__in=target_routes
                ).aggregate(total=Sum('amount'))['total'] or 0
                
                # Daily Collections
                daily_collection = CollectionPayment.objects.filter(
                ...
                day_closing = day_opening + daily_credit - daily_collection
                ...
                total_collection += daily_collection
                
                # Prepare for next day
                running_balance = day_closing
                current_date += datetime.timedelta(days=1)
                
            total_closing = running_balance

    # Excel Export
    if export_type == 'excel' and daily_data:
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        filename = f"Monthly_Outstanding_{from_date}_{to_date}.xlsx"
        response['Content-Disposition'] = f'attachment; filename={filename}'

        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Monthly Outstanding"

        # Headers
        headers = ['Date', 'Opening', 'Credit Amount', 'Collection', 'Closing']
        ws.append(headers)
        ...
        # Totals
        ws.append(['Total', '', total_credit, total_collection, total_closing])
        
        # Summary
        ws.append([]) # Empty row
        ws.append(['Summary'])
        ws.append(['Opening Outstanding', opening_outstanding])
        ws.append(['Credit Collection', total_collection])
        ws.append(['Outstanding Due (Last Month)', opening_outstanding - total_collection])
        
        ws.append(['Credit Sales', total_credit])
        ws.append(['Closing Outstanding', total_closing])
    ...
    context = {
        'data_filter': data_filter,
        'routes_instances': routes_instances,
        'salesmen_instances': salesmen_instances,
        'filter_data': filter_data,
    ...
    return render(request, 'sales_management/monthly_outstanding_report.html', context)


# ─────────────────────────────────────────────
# Customer Bottle Report
# ─────────────────────────────────────────────
def customer_bottle_report(request):
    """Page 1 – route-level summary: route | total bottles | view button."""
    from client_management.models import CustodyCustomItems, CustomerOutstanding, OutstandingProduct
    from master.models import RouteMaster
    ...
    for route in routes:
        custody_bottles = (
            CustodyCustomItems.objects.filter(
                custody_custom__customer__routes=route,
                custody_custom__customer__is_deleted=False,
                product__product_name__in=["5 Gallon", "5 Gallon Empty Bottle"],
            ).aggregate(total=Sum('quantity'))['total'] or 0
        )
        outstanding_bottles = (
            OutstandingProduct.objects.filter(
                customer_outstanding__customer__routes=route,
        ...
        total_bottles = custody_bottles + outstanding_bottles
        
        if total_bottles > 0:
            route_summary.append({
                'route': route,
                'custody_bottles': custody_bottles,
                'outstanding_bottles': outstanding_bottles,
                'total_bottles': total_bottles,
            })

    return render(request, 'sales_management/customer_bottle_report.html', {
        'route_summary': route_summary,
    })


def customer_bottle_route_detail(request, route_id):
    """Page 2 – per-customer breakdown for a given route, with search."""
    from client_management.models import CustodyCustomItems, CustomerOutstanding, OutstandingProduct
    from master.models import RouteMaster
    from accounts.models import Customers
    ...
    customers_qs = Customers.objects.filter(
        ...
        is_deleted=False,
        is_guest=False,
    ).order_by('custom_id', 'customer_name')

    if search:
        customers_qs = customers_qs.filter(
            Q(customer_name__icontains=search) | Q(custom_id__icontains=search)
        )

    # Prefetch custody stock in one query
    custody_map = {
        row['custody_custom__customer_id']: row['total']
        for row in CustodyCustomItems.objects.filter(
            custody_custom__customer__routes=route,
            custody_custom__customer__is_deleted=False,
            product__product_name__in=["5 Gallon", "5 Gallon Empty Bottle"],
        ).values('custody_custom__customer_id').annotate(total=Sum('quantity'))
    }

    # Prefetch outstanding empty bottles in one query
    outstanding_map = {
        ...
        for row in OutstandingProduct.objects.filter(
            ...
            customer_outstanding__customer__is_deleted=False,
            customer_outstanding__product_type='emptycan',
        ).values('customer_outstanding__customer_id').annotate(total=Sum('empty_bottle'))
    }

    customer_data = []
    total_custody = total_outstanding = total_bottles = 0
    idx = 1

    for customer in customers_qs:
        cid = customer.customer_id
        custody = custody_map.get(cid, 0)
        outstanding = outstanding_map.get(cid, 0)
        total = custody + outstanding
        ...
        if total > 0:
            ...
            customer_data.append({
                'sl_no': idx,
                'customer': customer,
                'custody_bottles': custody,
                'outstanding_bottles': outstanding,
                'total_bottles': total,
            })
            idx += 1

    return render(request, 'sales_management/customer_bottle_route_detail.html', {
        'route': route,
...
def customer_price_history_report(request):
    from accounts.models import CustomerPriceChange
    from master.models import RouteMaster
    import datetime
    ...
    if not request.GET:
        ...
        return render(request, 'sales_management/customer_price_history_report.html', context)

    data_filter = True
    
    # We query the CustomerPriceChange model for price histories
    price_history_qs = CustomerPriceChange.objects.filter(
    ...
    if route_names:
        filter_data['route_name'] = route_names
        if "all" not in [str(r).lower() for r in route_names]:
            price_history_qs = price_history_qs.filter(customer__routes__route_name__in=route_names)

    context = {
        'data_filter': data_filter,
        'routes_instances': routes_instances,
        'filter_data': filter_data,
        'price_history_data': price_history_qs.order_by('-created_date'),
    }

    return render(request, 'sales_management/customer_price_history_report.html', context)

def customer_price_history_report_excel(request):
    from accounts.models import CustomerPriceChange
    from master.models import RouteMaster
    import datetime
    import openpyxl
    ...
    price_history_qs = price_history_qs.order_by('-created_date')

    # generate openpyxl
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Price History Report"

    headers = ['Created Date', 'Route', 'Customer ID', 'Customer Name', 'Old Rate', 'New Rate']
    ws.append(headers)

    for cell in ws[1]:
        cell.font = Font(bold=True)

    for item in price_history_qs:
        ws.append([
            item.created_date.strftime('%d-%m-%Y, %I:%M %p') if item.created_date else '',
            item.customer.routes.route_name if item.customer and item.customer.routes else '',
            item.customer.custom_id if item.customer else '',
            item.customer.customer_name if item.customer else '',
            float(item.old_price) if item.old_price else 0.0,
            float(item.new_price) if item.new_price else 0.0,
        ])


    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = f'attachment; filename=customer_price_history_report_{from_date}_{to_date}.xlsx'
    wb.save(response)
    return response
...
# Shows daily bottle supply counts per customer for a route/month/year
# Includes cash supply, credit supply, and coupon supply counts
# ─────────────────────────────────────────────────────────────
def _get_delivery_report_data(selected_year, selected_month, selected_route_id, search_query=''):
    """Shared helper that builds the cross-tab data for a route/month."""
    import calendar
    from master.models import RouteMaster
    from accounts.models import Customers
    from django.db.models import Sum, Count
    from client_management.models import (
        CustomerSupply, CustomerSupplyItems,
        CustomerSupplyCoupon, CustomerSupplyDigitalCoupon,
        CustodyCustomItems, OutstandingProduct
    )
    ...
    if selected_route_id:
        try:
            route_obj = RouteMaster.objects.get(route_id=selected_route_id)
        except RouteMaster.DoesNotExist:
            return days, report_data, route_obj

    if route_obj:
        customers = (
            Customers.objects
            .filter(routes=route_obj, is_deleted=False, is_active=True)
        ...
        if search_query:
            ...
            customers = customers.filter(
                Q(customer_name__icontains=search_query) | Q(custom_id__icontains=search_query)
            )

        supplies_qs = (
            CustomerSupply.objects
            .filter(
                customer__routes=route_obj,
                customer__is_deleted=False,
                supply_date__year=selected_year,
                supply_date__month=selected_month,
            )
            .select_related('customer')
        )

        # Build lookup: {customer_id: {day: qty}}
        supply_lookup = {}
        for supply in supplies_qs:
            cust_id = supply.customer.customer_id
            day = supply.supply_date.day

            item_qty = (
                CustomerSupplyItems.objects
                .filter(customer_supply=supply)
                .aggregate(total=Sum('quantity'))['total'] or 0
            )

            manual = (
                CustomerSupplyCoupon.objects
                .filter(customer_supply=supply)
                .aggregate(lf=Count('leaf'), fl=Count('free_leaf'))
            )
            coupon_qty = (manual.get('lf') or 0) + (manual.get('fl') or 0)

            digital_qty = (
                CustomerSupplyDigitalCoupon.objects
                .filter(customer_supply=supply)
                .aggregate(total=Sum('count'))['total'] or 0
            )

            total_qty = item_qty + coupon_qty + digital_qty
            if cust_id not in supply_lookup:
                supply_lookup[cust_id] = {}
            supply_lookup[cust_id][day] = supply_lookup[cust_id].get(day, 0) + total_qty

        # Prefetch custody stock in one query
        custody_map = {
            row['custody_custom__customer_id']: row['total']
            for row in CustodyCustomItems.objects.filter(
                custody_custom__customer__routes=route_obj,
                custody_custom__customer__is_deleted=False,
                product__product_name__in=["5 Gallon", "5 Gallon Empty Bottle"],
            ).values('custody_custom__customer_id').annotate(total=Sum('quantity'))
        }
        ...
        outstanding_map = {
            row['customer_outstanding__customer_id']: row['total']
            for row in OutstandingProduct.objects.filter(
                customer_outstanding__customer__routes=route_obj,
                customer_outstanding__customer__is_deleted=False,
                customer_outstanding__product_type='emptycan',
            ).values('customer_outstanding__customer_id').annotate(total=Sum('empty_bottle'))
        }
        ...
        for customer in customers:
            ...
            day_totals = supply_lookup.get(cust_id, {})
            daily_counts = [day_totals.get(d, 0) for d in days]
            grand_total = sum(daily_counts)
            
            custody = custody_map.get(cust_id, 0)
            outstanding = outstanding_map.get(cust_id, 0)
            bottle_status = custody + outstanding

            report_data.append({
                'route_name': route_obj.route_name,
                'custom_id': customer.custom_id or '',
                'customer_name': customer.customer_name or '',
                'created_date': customer.created_date.strftime('%d-%m-%Y') if customer.created_date else '',
                'price': customer.rate or 0,
...
def detailed_delivery_report(request):
    import calendar
    from master.models import RouteMaster
    ...
    routes = RouteMaster.objects.all().order_by('route_name')
    days, report_data, route_obj = _get_delivery_report_data(
        selected_year, selected_month, selected_route_id, search_query
    )
    ...
    return render(request, 'sales_management/detailed_delivery_report.html', context)


def detailed_delivery_report_print(request):
    """Print-friendly version of Detailed Delivery Report."""
    import calendar
    from master.models import RouteMaster
    ...
    return render(request, 'sales_management/detailed_delivery_report_print.html', context)


def detailed_delivery_report_excel(request):
    import openpyxl
    import calendar
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Detailed Delivery Report"

    route_name = route_obj.route_name if route_obj else "All"
    month_name = calendar.month_name[selected_month]

    # Report Header
    ws.merge_cells('A1:AJ1')
    ws['A1'] = "Detailed Delivery Report"
    ws['A1'].font = Font(bold=True, size=14)
    ws['A1'].alignment = Alignment(horizontal='center')

    ws.merge_cells('A2:AJ2')
    ws['A2'] = f"Route: {route_name} | Month: {month_name} {selected_year}"
    ws['A2'].font = Font(bold=True)
    ws['A2'].alignment = Alignment(horizontal='center')

    # Table Headers
    headers = ['Sl No', 'Route', 'Cust ID', 'Customer Name', 'Created Date', 'Location','Price','Bottle Status'] + [str(d) for d in days] + ['Total']
    ws.append(headers)
    ...
    if report_data:
        for idx, row in enumerate(report_data, start=1):
            row_data = [
                ...
                row['price'],
                row['bottle_status'],
            ] + row['daily_counts'] + [row['total']]
            ws.append(row_data)

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = f'attachment; filename=detailed_delivery_report_{month_name}_{selected_year}.xlsx'
    wb.save(response)
    return response
