...
def stock_transfer_report(request):
    """
    Report view for showing stock transfers between vans.
    """
    instances = StockMovementProducts.objects.all().order_by('-stock_movement__created_date')
    ...
    if date_range:
        try:
            ...
            instances = instances.filter(stock_movement__created_date__date__range=[start_date, end_date])
        except ValueError:
    ...
    if search_query:
        instances = instances.filter(
            Q(product__product_name__icontains=search_query) |
            Q(stock_movement__from_van__van_make__icontains=search_query) |
            Q(stock_movement__to_van__van_make__icontains=search_query) |
            Q(stock_movement__salesman__username__icontains=search_query)
        )
    ...
    return render(request, 'van_management/stock_transfer_report.html', context)
